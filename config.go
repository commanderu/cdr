...
import (
	...
	_ "github.com/commanderu/cdrd/database/ffldb"
	"github.com/commanderu/cdrd/cdrutil"
	"github.com/commanderu/cdrd/mempool"
	"github.com/commanderu/cdrd/sampleconfig"
	flags "github.com/jessevdk/go-flags"
)

const (
	defaultConfigFilename        = "cdrd.conf"
	defaultDataDirname           = "data"
	defaultLogLevel              = "info"
	defaultLogDirname            = "logs"
	defaultLogFilename           = "cdrd.log"
	defaultMaxPeers              = 125
...
var (
	defaultHomeDir     = cdrutil.AppDataDir("cdrd", false)
...
// config defines the configuration options for cdrd.
//
...
type config struct {
	...
	RPCListeners         []string      `long:"rpclisten" description:"Add an interface/port to listen for RPC connections (default port: 37460, testnet: 137460)"`
	...
	MinRelayTxFee        float64       `long:"minrelaytxfee" description:"The minimum transaction fee in cdr/kB to be considered a non-zero fee."`
...
// createDefaultConfig copies the file sample-cdrd.conf to the given destination path,
...
//
// The above results in cdrd functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	// Update the home directory for cdrd if specified. Since the home
	...
	cfg.minRelayTxFee, err = cdrutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	cfg.miningAddrs = make([]cdrutil.Address, 0, len(cfg.GetWorkKeys)+
		len(cfg.MiningAddrs))
	for _, strAddr := range cfg.GetWorkKeys {
		addr, err := cdrutil.DecodeAddress(strAddr)
		if err != nil {
	...
	for _, oldDir := range oldTestNets {
		if fileExists(oldDir) {
			cdrdLog.Warnf("Block chain data from previous testnet"+
	...
	if configFileError != nil {
		cdrdLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// cdrdDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func cdrdDial(network, addr string) (net.Conn, error) {
...
// cdrdLookup returns the correct DNS lookup function to use depending on the
...
func cdrdLookup(host string) ([]net.IP, error) {
