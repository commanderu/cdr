...
func IsCoinBase(tx *cdrutil.Tx) bool {
...
func CheckTransactionSanity(tx *wire.MsgTx, params *chaincfg.Params) error {
	...
	// known as an atom.  One commanderu is a quantity of atoms as defined by
	...
	for _, txOut := range tx.TxOut {
		...
		// is detected and reported.  This is impossible for commanderu,
		...
		if totalAtom > cdrutil.MaxAmount {
			str := fmt.Sprintf("total value of all transaction "+
...
func checkProofOfStake(block *cdrutil.Block, posLimit int64) error {
...
func checkBlockSanity(block *cdrutil.Block, timeSource MedianTimeSource, flags BehaviorFlags, chainParams *chaincfg.Params) error {
	msgBlock := block.MsgBlock()
...
// sane before continuing with block processing.  These checks are context
// free.
func CheckBlockSanity(block *cdrutil.Block, timeSource MedianTimeSource, chainParams *chaincfg.Params) error {
...
// context free.
func CheckWorklessBlockSanity(block *cdrutil.Block, timeSource MedianTimeSource, chainParams *chaincfg.Params) error {
...
func (b *BlockChain) checkBlockContext(block *cdrutil.Block, prevNode *blockNode, flags BehaviorFlags) error {
	// The genesis block is valid by definition.
	if prevNode == nil {
		return nil
	}
...
//
// commanderu: Check the stake transactions to make sure they don't have this txid
// too.
func (b *BlockChain) checkDupTxs(txSet []*cdrutil.Tx, view *UtxoViewpoint) error {
...
// owner of the commanderu and therefore allowed to spend them.  As it checks the
// inputs, it also calculates the total fees for the transaction and returns
// that value.
//
// NOTE: The transaction MUST have already been sanity checked with the
// CheckTransactionSanity function prior to calling this function.
func CheckTransactionInputs(subsidyCache *SubsidyCache, tx *cdrutil.Tx, txHeight int64, utxoView *UtxoViewpoint, checkFraudProof bool, chainParams *chaincfg.Params) (int64, error) {
	msgTx := tx.MsgTx()
	...
	// -------------------------------------------------------------------
	// commanderu stake transaction testing.
	// -------------------------------------------------------------------
	...
	// -------------------------------------------------------------------
	// commanderu general transaction testing (and a few stake exceptions).
	// -------------------------------------------------------------------
	for idx, txIn := range msgTx.TxIn {
		...
		// commanderu is a quantity of atoms as defined by the AtomPerCoin
		// constant.
		...
		if originTxAtom > cdrutil.MaxAmount {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				originTxAtom, cdrutil.MaxAmount)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
...
// txscript.
func CountSigOps(tx *cdrutil.Tx, isCoinBaseTx bool, isSSGen bool) int {
	msgTx := tx.MsgTx()
...
func CountP2SHSigOps(tx *cdrutil.Tx, isCoinBaseTx bool, isStakeBaseTx bool, utxoView *UtxoViewpoint) (int, error) {
...
func checkNumSigOps(tx *cdrutil.Tx, utxoView *UtxoViewpoint, index int, txTree bool, cumulativeSigOps int) (int, error) {
	msgTx := tx.MsgTx()
	isSSGen := stake.IsSSGen(msgTx)
...
func checkStakeBaseAmounts(subsidyCache *SubsidyCache, height int64, params *chaincfg.Params, txs []*cdrutil.Tx, utxoView *UtxoViewpoint) error {
	for _, tx := range txs {
		msgTx := tx.MsgTx()
...
func getStakeBaseAmounts(txs []*cdrutil.Tx, utxoView *UtxoViewpoint) (int64, error) {
	totalInputs := int64(0)
	totalOutputs := int64(0)
	for _, tx := range txs {
		msgTx := tx.MsgTx()
...
func getStakeTreeFees(subsidyCache *SubsidyCache, height int64, params *chaincfg.Params, txs []*cdrutil.Tx, utxoView *UtxoViewpoint) (cdrutil.Amount, error) {
	totalInputs := int64(0)
	totalOutputs := int64(0)
	for _, tx := range txs {
		msgTx := tx.MsgTx()
	...
	return cdrutil.Amount(totalInputs - totalOutputs), nil
}
...
func (b *BlockChain) checkTransactionsAndConnect(subsidyCache *SubsidyCache, inputFees cdrutil.Amount, node *blockNode, txs []*cdrutil.Tx, utxoView *UtxoViewpoint, stxos *[]spentTxOut, txTree bool) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkConnectBlock(node *blockNode, block, parent *cdrutil.Block, utxoView *UtxoViewpoint, stxos *[]spentTxOut) error {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) CheckConnectBlockTemplate(block *cdrutil.Block) error {
	b.chainLock.Lock()
	...
	var nextBlockToDetach *cdrutil.Block
	...
	var prevAttachBlock *cdrutil.Block
