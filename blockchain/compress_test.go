...
func TestAmountCompression(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "0 cdr (sometimes used in nulldata)",
			uncompressed: 0,
			compressed:   0,
		},
		{
		...
		{
			name:         "0.00001 cdr (typical transaction fee)",
			uncompressed: 1000,
			compressed:   4,
		},
		{
			name:         "0.0001 cdr (typical transaction fee)",
			uncompressed: 10000,
			compressed:   5,
		},
		{
		...
		{
			name:         "21000000 cdr (max minted coins)",
...
func TestCompressedTxOut(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:          "pay-to-pubkey uncompressed 1 cdr, amount present",
			amount:        100000000,
			scriptVersion: 0,
			pkScript:      hexToBytes("4104192d74d0cb94344c9569c2e77901573d8d7903c3ebec3a957724895dca52c6b40d45264838c0bd96852662ce6a847b197376830160c6d2eb5e6a4c44d33f453eac"),
			compPkScript:  hexToBytes("04192d74d0cb94344c9569c2e77901573d8d7903c3ebec3a957724895dca52c6b4"),
