...
import (
	...
	"github.com/commanderu/cdrd/blockchain/chaingen"
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := cdrutil.Hash160(redeemScript)
...
// subsidy.
func additionalCoinbasePoW(amount cdrutil.Amount) func(*wire.MsgBlock) {
...
func additionalCoinbaseDev(amount cdrutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
		// Increase the first proof-of-work coinbase subsidy by the
		// provided amount.
...
func additionalSpendFee(fee cdrutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
func Generate(includeLargeReorg bool) (tests [][]TestInstance, err error) {
	...
	g.NextBlock("bbadtaxscript", outs[5], ticketOuts[5],
		func(b *wire.MsgBlock) {
			...
			p2shTaxAddr := addrs[0].(*cdrutil.AddressScriptHash)
			p2pkhTaxAddr, err := cdrutil.NewAddressPubKeyHash(
	...
	g.NextBlock("bv15", outs[9], ticketOuts[9], func(b *wire.MsgBlock) {
		ticketFee := cdrutil.Amount(2)
		ticketPrice := cdrutil.Amount(g.CalcNextReqStakeDifficulty(g.Tip()))
		ticketPrice--
	...
	// vector is not possible in commanderu since the block size is included in
	...
	// commanderu enforces the coinbase includes the block height to which it
	...
	g.NextBlock("bcb9", outs[19], ticketOuts[19], func(b *wire.MsgBlock) {
		b.Transactions[1].TxOut[0].Value = cdrutil.MaxAmount + 1
	})
	rejected(blockchain.ErrBadTxOutValue)
	...
	g.NextBlock("bcb10", outs[19], ticketOuts[19], func(b *wire.MsgBlock) {
		b.Transactions[1].TxOut[0].Value = cdrutil.MaxAmount
