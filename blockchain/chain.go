...
// BlockChain provides functions for working with the commanderu block chain.
...
type BlockChain struct {
	...
	mainchainBlockCache     map[chainhash.Hash]*cdrutil.Block
	mainchainBlockCacheSize int

	// These fields are related to checkpoint handling.  They are protected
	// by the chain lock.
	nextCheckpoint  *chaincfg.Checkpoint
	checkpointBlock *cdrutil.Block
...
// StakeVersions is a condensed form of a cdrutil.Block that is used to prevent
...
// exceeded.
func (b *BlockChain) addOrphanBlock(block *cdrutil.Block) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) fetchMainChainBlockByHash(hash *chainhash.Hash) (*cdrutil.Block, error) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) fetchBlockByHash(hash *chainhash.Hash) (*cdrutil.Block, error) {
	// Check orphan cache.
	b.orphanLock.RLock()
	...
	err := b.db.View(func(dbTx database.Tx) error {
		...
		block, err = cdrutil.NewBlockFromBytes(blockBytes)
		return err
	})
...
func (b *BlockChain) pushMainChainBlockCache(block *cdrutil.Block) {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBlock(node *blockNode, block, parent *cdrutil.Block, view *UtxoViewpoint, stxos []spentTxOut) error {
	...
	blockAndParent := []*cdrutil.Block{block, parent}
...
func (b *BlockChain) dropMainChainBlockCache(block *cdrutil.Block) {
	curHash := block.Hash()
	b.mainchainBlockCacheLock.Lock()
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) disconnectBlock(node *blockNode, block, parent *cdrutil.Block, view *UtxoViewpoint) error {
...
func countSpentOutputs(block *cdrutil.Block, parent *cdrutil.Block) int {
...
// adding the block.
func countNumberOfTransactions(block, parent *cdrutil.Block) uint64 {
	var numTxns uint64
	if headerApprovesParent(&block.MsgBlock().Header) {
...
func (b *BlockChain) reorganizeChain(detachNodes, attachNodes *list.List) error {
	...
	detachBlocks := make([]*cdrutil.Block, 0, detachNodes.Len())
	detachSpentTxOuts := make([][]spentTxOut, 0, detachNodes.Len())
	attachBlocks := make([]*cdrutil.Block, 0, attachNodes.Len())
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBestChain(node *blockNode, block, parent *cdrutil.Block, flags BehaviorFlags) (bool, error) {
...
type IndexManager interface {
	...
	// main chain.
	ConnectBlock(database.Tx, *cdrutil.Block, *cdrutil.Block, *UtxoViewpoint) error

	// DisconnectBlock is invoked when a block has been disconnected from
	// the main chain.
	DisconnectBlock(database.Tx, *cdrutil.Block, *cdrutil.Block, *UtxoViewpoint) error
}
...
func New(config *Config) (*BlockChain, error) {
	...
	b := BlockChain{
		...
		mainchainBlockCache:           make(map[chainhash.Hash]*cdrutil.Block),
