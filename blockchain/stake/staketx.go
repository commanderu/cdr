...
func AddrFromSStxPkScrCommitment(pkScript []byte,
	params *chaincfg.Params) (cdrutil.Address, error) {
	...
	if isP2SH {
		addr, err = cdrutil.NewAddressScriptHashFromHash(hashBytes, params)
	} else {
		addr, err = cdrutil.NewAddressPubKeyHash(hashBytes, params,
...
// ticket commitment pkScript.
func AmountFromSStxPkScrCommitment(pkScript []byte) (cdrutil.Amount, error) {
	if len(pkScript) < SStxPKHMinOutSize {
	...
	return cdrutil.Amount(binary.LittleEndian.Uint64(amtEncoded)), nil
}
