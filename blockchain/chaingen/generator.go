// Copyright (c) 2016-2018 The commanderu developers
...
func (s *SpendableOut) Amount() cdrutil.Amount {
	return s.amount
}
...
func makeSpendableOutForTxInternal(tx *wire.MsgTx, blockHeight, txIndex, txOutIndex uint32, tree int8) SpendableOut {
	return SpendableOut{
		...
		blockIndex:  txIndex,
		amount:      cdrutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
type Generator struct {
	...
	p2shOpTrueAddr   cdrutil.Address
...
func MakeGenerator(params *chaincfg.Params) (Generator, error) {
	...
	p2shOpTrueAddr, err := cdrutil.NewAddressScriptHash(opTrueScript, params)
	if err != nil {
...
// a single OP_TRUE.
func (g *Generator) P2shOpTrueAddr() cdrutil.Address {
...
func (g *Generator) calcFullSubsidy(blockHeight uint32) cdrutil.Amount {
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcPoWSubsidy(fullSubsidy cdrutil.Amount, blockHeight uint32, numVotes uint16) cdrutil.Amount {
	powProportion := cdrutil.Amount(g.params.WorkRewardProportion)
	totalProportions := cdrutil.Amount(g.params.TotalSubsidyProportions())
	...
	ticketsPerBlock := cdrutil.Amount(g.params.TicketsPerBlock)
	return (powSubsidy * cdrutil.Amount(numVotes)) / ticketsPerBlock
}
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcPoSSubsidy(heightVotedOn uint32) cdrutil.Amount {
	if int64(heightVotedOn+1) < g.params.StakeValidationHeight {
		return 0
	}

	fullSubsidy := g.calcFullSubsidy(heightVotedOn)
	posProportion := cdrutil.Amount(g.params.StakeRewardProportion)
	totalProportions := cdrutil.Amount(g.params.TotalSubsidyProportions())
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcDevSubsidy(fullSubsidy cdrutil.Amount, blockHeight uint32, numVotes uint16) cdrutil.Amount {
	devProportion := cdrutil.Amount(g.params.BlockTaxProportion)
	totalProportions := cdrutil.Amount(g.params.TotalSubsidyProportions())
	...
	// Reduce the subsidy according to the number of votes.
	ticketsPerBlock := cdrutil.Amount(g.params.TicketsPerBlock)
	return (devSubsidy * cdrutil.Amount(numVotes)) / ticketsPerBlock
}
...
func (g *Generator) addCoinbaseTxOutputs(tx *wire.MsgTx, blockHeight uint32, devSubsidy, powSubsidy cdrutil.Amount) {
...
func PurchaseCommitmentScript(addr cdrutil.Address, amount, voteFeeLimit, revocationFeeLimit cdrutil.Amount) []byte {
...
func (g *Generator) CreateTicketPurchaseTx(spend *SpendableOut, ticketPrice, fee cdrutil.Amount) *wire.MsgTx {
...
func (g *Generator) CreateVoteTx(voteBlock *wire.MsgBlock, ticketTx *wire.MsgTx, ticketBlockHeight, ticketBlockIndex uint32) *wire.MsgTx {
	...
	voteSubsidy := posSubsidy / cdrutil.Amount(g.params.TicketsPerBlock)
	ticketPrice := cdrutil.Amount(ticketTx.TxOut[0].Value)
...
// children nodes.  A diagram depicting how this works for commanderu transactions
...
// will be nil.
func buildMerkleTreeStore(transactions []*cdrutil.Tx) []*chainhash.Hash {
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	utilTxns := make([]*cdrutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, cdrutil.NewTx(tx))
	}
...
//
// This compact form is only used in commanderu to encode unsigned 256-bit numbers
...
func (g *Generator) CreateSpendTx(spend *SpendableOut, fee cdrutil.Amount) *wire.MsgTx {
...
// opTrueRedeemScript.
func (g *Generator) CreateSpendTxForTx(tx *wire.MsgTx, blockHeight, txIndex uint32, fee cdrutil.Amount) *wire.MsgTx {
...
func (g *Generator) NextBlock(blockName string, spend *SpendableOut, ticketSpends []SpendableOut, mungers ...func(*wire.MsgBlock)) *wire.MsgBlock {
	...
	ticketPrice := cdrutil.Amount(g.CalcNextRequiredStakeDifficulty())
	...
	if nextHeight > uint32(g.params.CoinbaseMaturity) {
		...
		// outputs.
		if ticketSpends != nil {
			const ticketFee = cdrutil.Amount(2)
	...
	var stakeTreeFees cdrutil.Amount
	for txIdx, tx := range stakeTxns {
		...
		var inputSum, outputSum cdrutil.Amount
		for _, txIn := range tx.TxIn {
			inputSum += cdrutil.Amount(txIn.ValueIn)
		}
		for _, txOut := range tx.TxOut {
			outputSum += cdrutil.Amount(txOut.Value)
		}
...
func (g *Generator) CreatePremineBlock(blockName string, additionalAmount cdrutil.Amount, mungers ...func(*wire.MsgBlock)) *wire.MsgBlock {
	...
	var totalSubsidy cdrutil.Amount
	for _, payout := range g.params.BlockOneLedger {
		payoutAddr, err := cdrutil.DecodeAddress(payout.Address)
		if err != nil {
			panic(err)
		}
		...
		totalSubsidy += cdrutil.Amount(payout.Amount)
	}
