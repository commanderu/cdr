...
//
// This function MUST be called with the chain lock held (for reads).
func (b *BlockChain) findPreviousCheckpoint() (*cdrutil.Block, error) {
	if b.noCheckpoints || len(b.chainParams.Checkpoints) == 0 {
		return nil, nil
	}
...
func isNonstandardTransaction(tx *cdrutil.Tx) bool {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) IsCheckpointCandidate(block *cdrutil.Block) (bool, error) {
	b.chainLock.RLock()
	defer b.chainLock.RUnlock()
