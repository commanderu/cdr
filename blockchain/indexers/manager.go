...
import (
	...
	"github.com/commanderu/cdrd/blockchain/internal/progresslog"
...
func dbIndexConnectBlock(dbTx database.Tx, indexer Indexer, block, parent *cdrutil.Block, view *blockchain.UtxoViewpoint) error {
...
func dbIndexDisconnectBlock(dbTx database.Tx, indexer Indexer, block, parent *cdrutil.Block, view *blockchain.UtxoViewpoint) error {
...
func (m *Manager) Init(chain *blockchain.BlockChain, interrupt <-chan struct{}) error {
	...
	var cachedParent *cdrutil.Block
	for height := lowestHeight + 1; height <= bestHeight; height++ {
		if interruptRequested(interrupt) {
			return errInterruptRequested
		}

		var block, parent *cdrutil.Block
		err = m.db.Update(func(dbTx database.Tx) error {
...
func makeUtxoView(dbTx database.Tx, block, parent *cdrutil.Block, interrupt <-chan struct{}) (*blockchain.UtxoViewpoint, error) {
	...
	for txIdx, tx := range parentRegularTxs {
		...
		for _, txIn := range tx.MsgTx().TxIn {
			...
			view.AddTxOuts(cdrutil.NewTx(originTx),
	...
	for _, tx := range block.STransactions() {
		...
		for i, txIn := range msgTx.TxIn {
			...
			view.AddTxOuts(cdrutil.NewTx(originTx), int64(wire.NullBlockHeight),
				wire.NullBlockIndex)
		}
...
//
// This is part of the blockchain.IndexManager interface.
func (m *Manager) ConnectBlock(dbTx database.Tx, block, parent *cdrutil.Block, view *blockchain.UtxoViewpoint) error {
...
//
// This is part of the blockchain.IndexManager interface.
func (m *Manager) DisconnectBlock(dbTx database.Tx, block, parent *cdrutil.Block, view *blockchain.UtxoViewpoint) error {
	// Call each of the currently active optional indexes with the block
