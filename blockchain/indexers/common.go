...
type Indexer interface {
	...
	ConnectBlock(dbTx database.Tx, block, parent *cdrutil.Block, view *blockchain.UtxoViewpoint) error

	// DisconnectBlock is invoked when the index manager is notified that a
	// block has been disconnected from the main chain.
	DisconnectBlock(dbTx database.Tx, block, parent *cdrutil.Block, view *blockchain.UtxoViewpoint) error
}
...
// be considered valid.
func approvesParent(block *cdrutil.Block) bool {
	return cdrutil.IsFlagSet16(block.MsgBlock().Header.VoteBits,
		cdrutil.BlockValid)
}
