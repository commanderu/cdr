...
func dbAddTxIndexEntries(dbTx database.Tx, block, parent *cdrutil.Block, blockID uint32) error {
	// As an optimization, allocate a single slice big enough to hold all
	// of the serialized transaction index entries for the block and
	// serialize them directly into the slice.  Then, pass the appropriate
	// subslice to the database to be written.  This approach significantly
	// cuts down on the number of required allocations.
	addEntries := func(txns []*cdrutil.Tx, txLocs []wire.TxLoc, blockID uint32) error {
		offset := 0
...
// block (if they were valid) and every stake transaction in the passed block.
func dbRemoveTxIndexEntries(dbTx database.Tx, block, parent *cdrutil.Block) error {
	removeEntries := func(txns []*cdrutil.Tx) error {
		for _, tx := range txns {
...
//
// This is part of the Indexer interface.
func (idx *TxIndex) ConnectBlock(dbTx database.Tx, block, parent *cdrutil.Block, view *blockchain.UtxoViewpoint) error {
...
//
// This is part of the Indexer interface.
func (idx *TxIndex) DisconnectBlock(dbTx database.Tx, block, parent *cdrutil.Block, view *blockchain.UtxoViewpoint) error {
