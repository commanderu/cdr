...
var (
	...
	// data.
	gbtCoinbaseAux = &cdrjson.GetBlockTemplateResultAux{
...
var (
	...
	ErrRPCUnimplemented = &cdrjson.RPCError{
	...
	ErrRPCNoWallet = &cdrjson.RPCError{
		Code:    cdrjson.ErrRPCNoWallet,
...
// list of commands that we recognize, but for which cdrd has no support because
// it lacks support for wallet functionality. For these commands the user
// should ask a connected instance of cdrwallet.
...
// not needed.
func rpcInternalError(errStr, context string) *cdrjson.RPCError {
	logStr := errStr
	if context != "" {
		logStr = context + ": " + errStr
	}
	rpcsLog.Error(logStr)
	return cdrjson.NewRPCError(cdrjson.ErrRPCInternal.Code, errStr)
}
...
func rpcInvalidError(fmtStr string, args ...interface{}) *cdrjson.RPCError {
	return cdrjson.NewRPCError(cdrjson.ErrRPCInvalidParameter,
...
func rpcDeserializationError(fmtStr string, args ...interface{}) *cdrjson.RPCError {
	return cdrjson.NewRPCError(cdrjson.ErrRPCDeserialization,
		fmt.Sprintf(fmtStr, args...))
}

// rpcRuleError is a convenience function to convert a
// rule error to an RPC error with the appropriate code set.
func rpcRuleError(fmtStr string, args ...interface{}) *cdrjson.RPCError {
	return cdrjson.NewRPCError(cdrjson.ErrRPCMisc,
		fmt.Sprintf(fmtStr, args...))
}
...
func rpcDuplicateTxError(fmtStr string, args ...interface{}) *cdrjson.RPCError {
	return cdrjson.NewRPCError(cdrjson.ErrRPCDuplicateTx,
		fmt.Sprintf(fmtStr, args...))
}
...
// context parameter is only used in the log message and may be empty if it's
// not needed.
func rpcAddressKeyError(fmtStr string, args ...interface{}) *cdrjson.RPCError {
	return cdrjson.NewRPCError(cdrjson.ErrRPCInvalidAddressOrKey,
		fmt.Sprintf(fmtStr, args...))
}

// rpcDecodeHexError is a convenience function for returning a nicely formatted
// RPC error which indicates the provided hex string failed to decode.
func rpcDecodeHexError(gotHex string) *cdrjson.RPCError {
	return cdrjson.NewRPCError(cdrjson.ErrRPCDecodeHexString,
...
// transaction hash.
func rpcNoTxInfoError(txHash *chainhash.Hash) *cdrjson.RPCError {
	return cdrjson.NewRPCError(cdrjson.ErrRPCNoTxInfo,
...
func rpcMiscError(message string) *cdrjson.RPCError {
	return cdrjson.NewRPCError(cdrjson.ErrRPCMisc, message)
}
...
// These commands will be implemented in cdrwallet.
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cdrjson.CreateRawTransactionCmd)
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount > cdrutil.MaxAmount {
			return nil, rpcInvalidError("Invalid amount: 0 >= %v "+
				"> %v", amount, cdrutil.MaxAmount)
		}

		// Decode the provided address.
		addr, err := cdrutil.DecodeAddress(encodedAddr)
		if err != nil {
		...
		atomic, err := cdrutil.NewAmount(amount)
		if err != nil {
			return nil, rpcInternalError(err.Error(),
				"New amount")
		}
...
func handleCreateRawSStx(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cdrjson.CreateRawSStxCmd)
	...
	for i, cout := range c.COuts {
		...
		addr, err := cdrutil.DecodeAddress(cout.Addr)
		if err != nil {
			return nil, rpcAddressKeyError("Could not decode "+
				"address: %v", err)
		}
		...
		pkScript, err := txscript.GenerateSStxAddrPush(addr,
			cdrutil.Amount(amountsCommitted[i]), 0x0000)
		if err != nil {
			return nil, rpcInternalError(err.Error(),
		...
		// Ensure amount is in the valid range for monetary amounts.
		if cout.ChangeAmt < 0 || cout.ChangeAmt > cdrutil.MaxAmount {
			return nil, rpcInvalidError("Invalid change amount: 0 "+
				"> %v > %v", cout.ChangeAmt, cdrutil.MaxAmount)
		}

		// Decode the provided address.
		addr, err = cdrutil.DecodeAddress(cout.ChangeAddr)
		if err != nil {
			return nil, rpcAddressKeyError("Wrong network: %v",
				addr)
		}
...
func handleCreateRawSSGenTx(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cdrjson.CreateRawSSGenTxCmd)
	...
	for i, ssgenPkh := range ssgenPkhs {
		...
		if ssgenCalcAmts[i] <= 0 ||
			ssgenCalcAmts[i] > cdrutil.MaxAmount {
...
func handleCreateRawSSRtx(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cdrjson.CreateRawSSRtxCmd)
	...
	var feeAmt cdrutil.Amount
	if c.Fee != nil {
		var err error
		feeAmt, err = cdrutil.NewAmount(*c.Fee)
		if err != nil {
	...
	for i, ssrtxPkh := range ssrtxPkhs {
		// Ensure amount is in the valid range for monetary amounts.
		if sstxAmts[i] <= 0 || sstxAmts[i] > cdrutil.MaxAmount {
			return nil, rpcInvalidError("Invalid SSTx amount: 0 >="+
				" %v > %v", sstxAmts[i] <= 0, cdrutil.MaxAmount)
		}
...
// transaction.
func createVinList(mtx *wire.MsgTx) []cdrjson.Vin {
	// Coinbase transactions only have a single txin by definition.
	vinList := make([]cdrjson.Vin, len(mtx.TxIn))
	if blockchain.IsCoinBaseTx(mtx) {
		txIn := mtx.TxIn[0]
		vinEntry := &vinList[0]
		vinEntry.Coinbase = hex.EncodeToString(txIn.SignatureScript)
		vinEntry.Sequence = txIn.Sequence
		vinEntry.AmountIn = cdrutil.Amount(txIn.ValueIn).ToCoin()
	...
	for i, txIn := range mtx.TxIn {
		...
		vinEntry.BlockHeight = txIn.BlockHeight
		vinEntry.BlockIndex = txIn.BlockIndex
		vinEntry.ScriptSig = &cdrjson.ScriptSig{
			Asm: disbuf,
...
// transaction.
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []cdrjson.Vout {

	txType := stake.DetermineTxType(mtx)
	voutList := make([]cdrjson.Vout, 0, len(mtx.TxOut))
	for i, v := range mtx.TxOut {
		...
		var vout cdrjson.Vout
		voutSPK := &vout.ScriptPubKey
		vout.N = uint32(i)
		vout.Value = cdrutil.Amount(v.Value).ToCoin()
		...
		if commitAmt != nil {
			voutSPK.CommitAmt = cdrjson.Float64(commitAmt.ToCoin())
		}
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx, txHash string, blkIdx uint32, blkHeader *wire.BlockHeader, blkHash string, blkHeight int64, confirmations int64) (*cdrjson.TxRawResult, error) {
	mtxHex, err := messageToHex(mtx)
	if err != nil {
		return nil, err
	}
...
func handleDecodeRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cdrjson.DecodeRawTransactionCmd)
	...
	txReply := cdrjson.TxRawDecodeResult{
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	p2sh, err := cdrutil.NewAddressScriptHash(script, s.server.chainParams)
	if err != nil {
		return nil, rpcInternalError(err.Error(),
	...
	reply := cdrjson.DecodeScriptResult{
		Asm:       disbuf,
...
func handleEstimateStakeDiff(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cdrjson.EstimateStakeDiffCmd)
	...
	if c.Tickets != nil {
		...
		userEstFlt := cdrutil.Amount(userEst).ToCoin()
		userEstFltPtr = &userEstFlt
	}

	return &cdrjson.EstimateStakeDiffResult{
		Min:      cdrutil.Amount(min).ToCoin(),
		Max:      cdrutil.Amount(max).ToCoin(),
		Expected: cdrutil.Amount(expected).ToCoin(),
		User:     userEstFltPtr,
	}, nil
}
...
func handleExistsAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*cdrjson.ExistsAddressCmd)

	// Attempt to decode the supplied address.
	addr, err := cdrutil.DecodeAddress(c.Address)
	if err != nil {
...
func handleExistsAddresses(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*cdrjson.ExistsAddressesCmd)
	addresses := make([]cdrutil.Address, len(c.Addresses))
	for i := range c.Addresses {
		// Attempt to decode the supplied address.
		addr, err := cdrutil.DecodeAddress(c.Addresses[i])
		if err != nil {
			return nil, rpcAddressKeyError("Could not decode "+
				"address: %v", err)
		}
...
func handleExistsMissedTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cdrjson.ExistsMissedTicketsCmd)

	hashes, err := cdrjson.DecodeConcatenatedHashes(c.TxHashBlob)
	if err != nil {
		return nil, err
	}
...
func handleExistsExpiredTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cdrjson.ExistsExpiredTicketsCmd)
...
func handleExistsLiveTicket(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cdrjson.ExistsLiveTicketCmd)
...
func handleExistsLiveTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cdrjson.ExistsLiveTicketsCmd)
...
func handleExistsMempoolTxs(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cdrjson.ExistsMempoolTxsCmd)
...
func handleGetAddedNodeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cdrjson.GetAddedNodeInfoCmd)

	// Retrieve a list of persistent (added) peers from the commanderu server
	...
	results := make([]*cdrjson.GetAddedNodeInfoResult, 0, len(peers))
	for _, peer := range peers {
		// Set the "address" of the peer which could be an ip address
		// or a domain name.
		var result cdrjson.GetAddedNodeInfoResult
		result.AddedNode = peer.Addr()
		result.Connected = cdrjson.Bool(peer.Connected())
		...
		addrs := make([]cdrjson.GetAddedNodeInfoResultAddr, 0,
			len(ipList))
		for _, ip := range ipList {
			var addr cdrjson.GetAddedNodeInfoResultAddr
			addr.Address = ip
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	sbitsFloat := float64(blockHeader.SBits) / cdrutil.AtomsPerCoin
	blockReply := cdrjson.GetBlockVerboseResult{
		Hash:          c.Hash,
	...
	if c.VerboseTx == nil || !*c.VerboseTx {
		...
		rawTxns := make([]cdrjson.TxRawResult, len(txns))
		for i, tx := range txns {
		...
		rawSTxns := make([]cdrjson.TxRawResult, len(stxns))
...
func handleGetBlockHash(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cdrjson.GetBlockHashCmd)
	hash, err := s.chain.BlockHashByHeight(c.Index)
	if err != nil {
		return nil, &cdrjson.RPCError{
			Code: cdrjson.ErrRPCOutOfRange,
...
func handleGetBlockHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cdrjson.GetBlockHeaderCmd)
	...
	blockHeaderReply := cdrjson.GetBlockHeaderVerboseResult{
		Hash:          c.Hash,
		Confirmations: confirmations,
		Version:       blockHeader.Version,
		PreviousHash:  blockHeader.PrevBlock.String(),
		...
		PoolSize:      blockHeader.PoolSize,
		Bits:          strconv.FormatInt(int64(blockHeader.Bits), 16),
		SBits:         cdrutil.Amount(blockHeader.SBits).ToCoin(),
...
func handleGetBlockSubsidy(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cdrjson.GetBlockSubsidyCmd)
	...
	rep := cdrjson.GetBlockSubsidyResult{
		Developer: dev,
		PoS:       pos,
		PoW:       pow,
		Total:     total,
...
// state as a cdrjson.GetBlockTemplateResult that is ready to be encoded to
// JSON and returned to the caller.
//
// This function MUST be called with the state locked.
func (state *gbtWorkState) blockTemplateResult(bm *blockManager, useCoinbaseValue bool, submitOld *bool) (*cdrjson.GetBlockTemplateResult, error) {
	...
	transactions := make([]cdrjson.GetBlockTemplateResultTx, 0, numTx-1)
	...
	for i, tx := range msgBlock.Transactions {
		...
		resultTx := cdrjson.GetBlockTemplateResultTx{
	...
	stransactions := make([]cdrjson.GetBlockTemplateResultTx, 0, numSTx)
	...
	reply := cdrjson.GetBlockTemplateResult{
...
func handleGetBlockTemplateRequest(s *rpcServer, request *cdrjson.TemplateRequest, closeChan <-chan struct{}) (interface{}, error) {
	...
	if !cfg.SimNet && s.server.ConnectedCount() == 0 {
		return nil, &cdrjson.RPCError{
			...
			Message: "commanderu is not connected",
		}
	}
	...
	if currentHeight != 0 && !s.server.blockManager.IsCurrent() {
		return nil, &cdrjson.RPCError{
			Code:    cdrjson.ErrRPCClientInInitialDownload,
			Message: "commanderu is downloading blocks...",
		}
	}
...
// reasons.
// TODO commanderu pop in the new errors from blockchain cj
...
func handleGetBlockTemplateProposal(s *rpcServer, request *cdrjson.TemplateRequest) (interface{}, error) {
...
func handleGetBlockTemplate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*cdrjson.GetBlockTemplateCmd)
	request := c.Request
...
func handleGetCFilterHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*cdrjson.GetCFilterHeaderCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetHeaders(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cdrjson.GetHeadersCmd)
	blockLocators, err := cdrjson.DecodeConcatenatedHashes(c.BlockLocators)
	if err != nil {
		// Already a *cdrjson.RPCError
		return nil, err
	}
	...
	return &cdrjson.GetHeadersResult{Headers: hexBlockHeaders}, nil
}
...
func handleGetMempoolInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &cdrjson.GetMempoolInfoResult{
...
func handleGetMiningInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	gnhpsCmd := cdrjson.NewGetNetworkHashPSCmd(nil, nil)
	...
	result := cdrjson.GetMiningInfoResult{
		Blocks:           best.Height,
...
func handleGetNetTotals(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	reply := &cdrjson.GetNetTotalsResult{
...
func handleGetNetworkHashPS(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*cdrjson.GetNetworkHashPSCmd)
...
func handleGetPeerInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	infos := make([]*cdrjson.GetPeerInfoResult, 0, len(peers))
	for _, p := range peers {
		statsSnap := p.StatsSnapshot()
		info := &cdrjson.GetPeerInfoResult{
			ID:             statsSnap.ID,
			Addr:           statsSnap.Addr,
...
func handleGetRawMempool(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cdrjson.GetRawMempoolCmd)
	...
	if c.TxType != nil {
		switch cdrjson.GetRawMempoolTxTypeCmd(*c.TxType) {
		case cdrjson.GRMRegular:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeRegular
		case cdrjson.GRMTickets:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeSStx
		case cdrjson.GRMVotes:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeSSGen
		case cdrjson.GRMRevocations:
			filterType = new(stake.TxType)
...
func handleGetRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cdrjson.GetRawTransactionCmd)
...
func handleGetStakeDifficulty(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if err != nil {
		...
		return nil, &cdrjson.RPCError{
			Code:    cdrjson.ErrRPCDifficulty,
			Message: "Error getting stake difficulty: " + err.Error(),
		}
	}
	currentSdiff := cdrutil.Amount(blockHeader.SBits)
	...
	nextSdiffAmount := cdrutil.Amount(nextSdiff)

	sDiffResult := &cdrjson.GetStakeDifficultyResult{
...
func convertVersionMap(m map[int]int) []cdrjson.VersionCount {
	sorted := make([]cdrjson.VersionCount, 0, len(m))
	...
	for _, v := range order {
		sorted = append(sorted, cdrjson.VersionCount{Version: uint32(v),
			Count: uint32(m[v])})
	}
...
func handleGetStakeVersionInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	count := int32(1)
	c, ok := cmd.(*cdrjson.GetStakeVersionInfoCmd)
	if !ok {
	...
	result := cdrjson.GetStakeVersionInfoResult{
		CurrentHeight: snapshot.Height,
		Hash:          snapshot.Hash.String(),
		Intervals:     make([]cdrjson.VersionInterval, 0, count),
	}
	...
	for i := int32(0); i < count; i++ {
		...
		versionInterval := cdrjson.VersionInterval{
			StartHeight:  endHeight,
...
func handleGetStakeVersions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cdrjson.GetStakeVersionsCmd)
	...
	result := cdrjson.GetStakeVersionsResult{
		StakeVersions: make([]cdrjson.StakeVersions, 0, len(sv)),
	}
	for _, v := range sv {
		nsv := cdrjson.StakeVersions{
			Hash:         v.Hash.String(),
			Height:       v.Height,
			BlockVersion: v.BlockVersion,
			StakeVersion: v.StakeVersion,
			Votes: make([]cdrjson.VersionBits, 0,
				len(v.Votes)),
		}
		for _, vote := range v.Votes {
			nsv.Votes = append(nsv.Votes,
				cdrjson.VersionBits{Version: vote.Version,
					Bits: vote.Bits})
		}
...
func handleGetVoteInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c, ok := cmd.(*cdrjson.GetVoteInfoCmd)
	if !ok {
		return nil, rpcInvalidError("Invalid type: %T", c)
	}
	...
	// Assemble JSON result.
	result := cdrjson.GetVoteInfoResult{
		CurrentHeight: snapshot.Height,
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if txFromMempool != nil {
		...
		if c.Vout > uint32(len(mtx.TxOut)-1) {
			return nil, &cdrjson.RPCError{
				Code: cdrjson.ErrRPCInvalidTxVout,
	...
	txOutReply := &cdrjson.GetTxOutResult{
		BestBlock:     bestBlockHash,
		Confirmations: confirmations,
		Value:         cdrutil.Amount(value).ToUnit(cdrutil.AmountCoin),
		Version:       int32(txVersion),
		ScriptPubKey: cdrjson.ScriptPubKeyResult{
			Asm:       disbuf,
...
func handleGetWorkSubmission(s *rpcServer, hexData string) (interface{}, error) {
	...
	// blockchain.
	tempBlock := cdrutil.NewBlockDeepCopy(blockInfo.msgBlock)
...
func handleLiveTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return cdrjson.LiveTicketsResult{Tickets: ltString}, nil
}
...
func handleMissedTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return cdrjson.MissedTicketsResult{Tickets: mtString}, nil
}
...
// passed transaction.
func createVinListPrevOut(s *rpcServer, mtx *wire.MsgTx, chainParams *chaincfg.Params, vinExtra bool, filterAddrMap map[string]struct{}) ([]cdrjson.VinPrevOut, error) {
	// Coinbase transactions only have a single txin by definition.
	if blockchain.IsCoinBaseTx(mtx) {
		...
		txIn := mtx.TxIn[0]
		vinList := make([]cdrjson.VinPrevOut, 1)
		vinList[0].Coinbase = hex.EncodeToString(txIn.SignatureScript)
		amountIn := cdrutil.Amount(txIn.ValueIn).ToCoin()
	...
	vinList := make([]cdrjson.VinPrevOut, 0, len(mtx.TxIn))
	...
	for i, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &cdrjson.PrevOut{
				Addresses: encodedAddrs,
				Value:     cdrutil.Amount(originTxOut.Value).ToCoin(),
			}
		}
	}
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr cdrutil.Address, numToSkip, numRequested uint32) ([]*cdrutil.Tx, uint32) {
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*cdrjson.SearchRawTransactionsCmd)
	vinExtra := false
	if c.VinExtra != nil {
	...
	srtList := make([]cdrjson.SearchRawTransactionsResult, len(addressTxns))
	for i := range addressTxns {
...
func handleSendRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cdrjson.SendRawTransactionCmd)
...
func min(s []cdrutil.Amount) cdrutil.Amount {
	if len(s) == 0 {
		return 0
	}
...
func max(s []cdrutil.Amount) cdrutil.Amount {
	max := cdrutil.Amount(0)
	for i := range s {
		if s[i] > max {
			max = s[i]
		}
...
func mean(s []cdrutil.Amount) cdrutil.Amount {
	sum := cdrutil.Amount(0)
	for i := range s {
		sum += s[i]
	}
	...
	return sum / cdrutil.Amount(len(s))
}

// median gets the median amount from a slice of amounts.
func median(s []cdrutil.Amount) cdrutil.Amount {
	if len(s) == 0 {
		return 0
	}

	sort.Sort(cdrutil.AmountSorter(s))
...
func stdDev(s []cdrutil.Amount) cdrutil.Amount {
	var total float64
	mean := mean(s)
	...
	amt, _ := cdrutil.NewAmount(math.Sqrt(v))
...
// memory pool.
func feeInfoForMempool(s *rpcServer, txType stake.TxType) *cdrjson.FeeInfoMempool {
	txDs := s.server.txMemPool.TxDescs()
	ticketFees := make([]cdrutil.Amount, 0, len(txDs))
	for _, txD := range txDs {
		if txD.Type == txType {
			feePerKb := (cdrutil.Amount(txD.Fee)) * 1000 /
				cdrutil.Amount(txD.Tx.MsgTx().SerializeSize())
			ticketFees = append(ticketFees, feePerKb)
		}
	}

	return &cdrjson.FeeInfoMempool{
...
// properly set.
func calcFeePerKb(tx *cdrutil.Tx) cdrutil.Amount {
	var in cdrutil.Amount
	for _, txIn := range tx.MsgTx().TxIn {
		in += cdrutil.Amount(txIn.ValueIn)
	}
	var out cdrutil.Amount
	for _, txOut := range tx.MsgTx().TxOut {
		out += cdrutil.Amount(txOut.Value)
	}

	return ((in - out) * 1000) / cdrutil.Amount(tx.MsgTx().SerializeSize())
}

// feeInfoForBlock fetches the ticket fee information for a given tx type in a
// block.
func ticketFeeInfoForBlock(s *rpcServer, height int64, txType stake.TxType) (*cdrjson.FeeInfoBlock, error) {
	...
	txFees := make([]cdrutil.Amount, txNum)
	itr := 0
...
// from [start, end).
func ticketFeeInfoForRange(s *rpcServer, start int64, end int64, txType stake.TxType) (*cdrjson.FeeInfoWindow, error) {
	hashes, err := s.chain.HeightRange(start, end)
	if err != nil {
		return nil, err
	}

	var txFees []cdrutil.Amount
	for i := range hashes {
...
func handleTicketFeeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cdrjson.TicketFeeInfoCmd)
	...
	var feeInfoBlocks []cdrjson.FeeInfoBlock
	blocks := uint32(0)
	if c.Blocks != nil {
		blocks = *c.Blocks
	}
	...
	var feeInfoWindows []cdrjson.FeeInfoWindow
	windows := uint32(0)
	if c.Windows != nil {
		windows = *c.Windows
	}
	...
	return &cdrjson.TicketFeeInfoResult{
...
func handleTicketsForAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cdrjson.TicketsForAddressCmd)
	...
	reply := &cdrjson.TicketsForAddressResult{
...
func handleTicketVWAP(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return cdrutil.Amount(vwap).ToCoin(), nil
}
...
func handleTxFeeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	var feeInfoRange cdrjson.FeeInfoRange
	...
	feeInfoRange = cdrjson.FeeInfoRange{
	...
	return &cdrjson.TxFeeInfoResult{
		FeeInfoMempool: *feeInfoMempool,
		FeeInfoBlocks:  feeInfoBlocks,
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cdrjson.ValidateAddressCmd)
	result := cdrjson.ValidateAddressChainResult{}
	addr, err := cdrutil.DecodeAddress(c.Address)
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*cdrjson.VerifyMessageCmd)
	...
	if wasCompressed {
		serializedPK = cdrPK.SerializeCompressed()
	} else {
		serializedPK = cdrPK.SerializeUncompressed()
	}
	address, err := cdrutil.NewAddressSecpPubKey(serializedPK,
		activeNetParams.Params)
	if err != nil {
...
func handleVersion(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	result := map[string]cdrjson.VersionResult{
		"cdrdjsonrpcapi": {
...
func (s *rpcServer) standardCmdResult(cmd *parsedRPCCmd, closeChan <-chan struct{}) (interface{}, error) {
	...
	return nil, cdrjson.ErrRPCMethodNotFound
handled:
...
func parseCmd(request *cdrjson.Request) *parsedRPCCmd {
	...
	if err != nil {
		...
		if jerr, ok := err.(cdrjson.Error); ok &&
			jerr.Code == cdrjson.ErrUnregisteredMethod {
			parsedCmd.err = cdrjson.ErrRPCMethodNotFound
			return &parsedCmd
		}
...
// type *cdrjson.RPCError to the appropriate type as needed.
func createMarshalledReply(rpcVersion string, id interface{}, result interface{}, replyErr error) ([]byte, error) {
	...
	if replyErr != nil {
		if jErr, ok := replyErr.(*cdrjson.RPCError); ok {
			jsonErr = jErr
		} else {
	...
	return cdrjson.MarshalResponse(rpcVersion, id, result, jsonErr)
}

// processRequest determines the incoming request type (single or batched),
// parses it and returns a marshalled response.
func (s *rpcServer) processRequest(request *cdrjson.Request, isAdmin bool, closeChan <-chan struct{}) []byte {
...
func (s *rpcServer) jsonRPCRead(w http.ResponseWriter, r *http.Request, isAdmin bool) {
	...
	if !batchedRequest {
		...
		if err != nil {
			...
			resp, err = cdrjson.MarshalResponse("1.0", nil, nil, jsonErr)
			if err != nil {
	...
	if batchedRequest {
		...
		if err != nil {
			...
			resp, err = cdrjson.MarshalResponse("2.0", nil, nil, jsonErr)
			if err != nil {
				rpcsLog.Errorf("Failed to create reply: %v", err)
			}
			...
			if len(batchedRequests) > 0 {
				...
				for _, entry := range batchedRequests {
					...
					if err != nil {
						...
						resp, err = cdrjson.MarshalResponse("", nil, nil, jsonErr)
						if err != nil {
							rpcsLog.Errorf("Failed to create reply: %v", err)
						}
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="cdrd RPC"`)
