...
For the complete details of the commanderu protocol, see the official wiki entry
...
supported commanderu messages to and from the wire.  This package does not deal
with the specifics of message handling such as what to do when a message is
received.  This provides the caller with a high level of flexibility.

commanderu Message Overview

The commanderu protocol consists of exchanging messages between peers.  Each
message is preceded by a header which identifies information about it such as
which commanderu network it is a part of, its type, how big it is, and a checksum
...
of the currently supported commanderu messages are provided.  For these supported
messages, all of the details of marshalling and unmarshalling to and from the
wire using commanderu encoding are handled so the caller doesn't have to concern
themselves with the specifics.

Message Interaction

The following provides a quick summary of how the commanderu messages are intended
to interact with one another.  As stated above, these interactions are not
directly handled by this package.  For more in-depth details about the
appropriate interactions, see the official commanderu protocol wiki entry at
...
and write commanderu messages.  The following sections provide a quick overview of
...
The commanderu network is a magic number which is used to identify the start of a
message and which commanderu network the message applies to.  This package provides
...
As discussed in the commanderu message overview section, this package reads
and writes commanderu messages using a generic interface named Message.  In
...
In order to unmarshall commanderu messages from the wire, use the ReadMessage
function.  It accepts any io.Reader, but typically this will be a net.Conn to
a remote node running a commanderu peer.  Example syntax is:

	// Reads and validates the next commanderu message from conn using the
	// protocol version pver and the commanderu network btcnet.  The returns
...
In order to marshall commanderu messages to the wire, use the WriteMessage
function.  It accepts any io.Writer, but typically this will be a net.Conn to
a remote node running a commanderu peer.  Example syntax to request addresses
from a remote peer is:
	...
	// pver, and the commanderu network btcnet.  The return is a possible
	// error.
