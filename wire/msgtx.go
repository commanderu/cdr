...
// OutPoint defines a commanderu data type that is used to track previous
// transaction outputs.
...
// NewOutPoint returns a new commanderu transaction outpoint point with the
...
// TxIn defines a commanderu transaction input.
type TxIn struct {
	// Non-witness
...
// NewTxIn returns a new commanderu transaction input with the provided
...
// TxOut defines a commanderu transaction output.
type TxOut struct {
	Value    int64
	Version  uint16
	PkScript []byte
...
// NewTxOut returns a new commanderu transaction output with the provided
...
// MsgTx implements the Message interface and represents a commanderu tx message.
...
func writeTxScriptsToMsgTx(msg *MsgTx, totalScriptSize uint64, serType TxSerializeType) {
	...
	//
	// Closures around writing the TxIn and TxOut scripts are used in commanderu
...
// BtcDecode decodes r using the commanderu protocol encoding into the receiver.
...
// in that BtcDecode decodes from the commanderu wire protocol as it was sent
...
// BtcEncode encodes the receiver to w using the commanderu protocol encoding.
// This is part of the Message interface implementation.
...
// encodes the transaction to the commanderu wire protocol in order to be sent
// across the network.  The wire encoding can technically differ depending on
// the protocol version and doesn't even really need to match the format of a
// stored transaction at all.  As of the time this comment was written, the
// encoded transaction is the same in both instances, but there is a distinct
...
// NewMsgTx returns a new commanderu tx message that conforms to the Message
...
// WriteOutPoint encodes op to the commanderu protocol encoding for an OutPoint
// to w.
...
// writeTxInPrefixs encodes ti to the commanderu protocol encoding for a transaction
...
// writeTxWitness encodes ti to the commanderu protocol encoding for a transaction
...
// writeTxOut encodes to into the commanderu protocol encoding for a transaction
// output (TxOut) to w.
