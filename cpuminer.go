...
func (m *CPUMiner) submitBlock(block *cdrutil.Block) bool {
	...
	minrLog.Infof("Block submitted via CPU miner accepted (hash %s, "+
		...
		cdrutil.Amount(coinbaseTxGenerated))
	return true
}
...
out:
	for {
		...
		// Hacks to make cdr work with commanderu PoC (simnet only)
		...
		if m.solveBlock(template.Block, ticker, quit) {
			block := cdrutil.NewBlock(template.Block)
			m.submitBlock(block)
