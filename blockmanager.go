...
// blockMsg packages a commanderu block message and the peer it came from together
...
// invMsg packages a commanderu inv message and the peer it came from together
// so the block handler has access to that information.
type invMsg struct {
	inv  *wire.MsgInv
	peer *serverPeer
...
// headersMsg packages a commanderu headers message and the peer it came from
...
// txMsg packages a commanderu tx message and the peer it came from together
// so the block handler has access to that information.
type txMsg struct {
	tx   *cdrutil.Tx
	peer *serverPeer
...
func (b *blockManager) checkBlockForHiddenVotes(block *cdrutil.Block) {
	var votesFromBlock []*cdrutil.Tx
	...
	var newVotes []*cdrutil.Tx
	var oldTickets []*cdrutil.Tx
	var oldRevocations []*cdrutil.Tx
	oldVoteMap := make(map[chainhash.Hash]struct{},
		int(b.server.chainParams.TicketsPerBlock))
	if template != nil {
		templateBlock := cdrutil.NewBlock(template.Block)
	...
	updatedTxTreeStake := make([]*cdrutil.Tx, 0,
	...
	var updatedTxTreeRegular []*cdrutil.Tx
...
func (b *blockManager) handleNotifyMsg(notification *blockchain.Notification) {
	...
	case blockchain.NTBlockConnected:
		blockSlice, ok := notification.Data.([]*cdrutil.Block)
		if !ok {
		...
		// below.
		txTreeRegularValid := cdrutil.IsFlagSet16(block.MsgBlock().Header.VoteBits,
			cdrutil.BlockValid)
...
// queue.
func (b *blockManager) QueueTx(tx *cdrutil.Tx, sp *serverPeer) {
...
func (b *blockManager) QueueBlock(block *cdrutil.Block, sp *serverPeer) {
...
func (b *blockManager) ProcessBlock(block *cdrutil.Block, flags blockchain.BehaviorFlags) (bool, error) {
...
func (b *blockManager) ProcessTransaction(tx *cdrutil.Tx, allowOrphans bool,
	rateLimit bool, allowHighFees bool) ([]*cdrutil.Tx, error) {
...
// pool.
func (b *blockManager) TicketPoolValue() (cdrutil.Amount, error) {
...
// newBlockManager returns a new commanderu block manager.
...
func warnMultipleDBs() {
	...
	if len(duplicateDbPaths) > 0 {
		...
		cdrdLog.Warnf("WARNING: There are multiple block chain databases "+
...
func loadBlockDB() (database.DB, error) {
	...
	if cfg.DbType == "memdb" {
		cdrdLog.Infof("Creating block database in memory.")
	...
	cdrdLog.Infof("Loading block database from '%s'", dbPath)
	...
	cdrdLog.Info("Block database loaded")
	return db, nil
}
