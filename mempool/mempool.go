...
import (
	...
	"github.com/commanderu/cdrd/blockchain/indexers"
	...
	"github.com/commanderu/cdrd/mining"
...
const (
	...
	MinHighPriority = cdrutil.AtomsPerCoin * 144.0 / 250
	...
	// cdr/kB, this results in a maximum allowed high fee of 1 cdr/kB.
...
type Config struct {
	...
	FetchUtxoView func(*cdrutil.Tx, bool) (*blockchain.UtxoViewpoint, error)

	// BlockByHash defines the function use to fetch the block identified
	// by the given hash.
	BlockByHash func(*chainhash.Hash) (*cdrutil.Block, error)
	...
	// utxo view.
	CalcSequenceLock func(*cdrutil.Tx, *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error)
...
type TxPool struct {
	...
	orphans       map[chainhash.Hash]*cdrutil.Tx
	orphansByPrev map[chainhash.Hash]map[chainhash.Hash]*cdrutil.Tx
	outpoints     map[wire.OutPoint]*cdrutil.Tx
...
func (mp *TxPool) insertVote(ssgen *cdrutil.Tx) error {
	msgTx := ssgen.MsgTx()
	...
	vote := cdrutil.IsFlagSet16(voteBits, cdrutil.BlockValid)
...
// TODO Pruning of the votes map commanderu
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addOrphan(tx *cdrutil.Tx) {
	...
	for _, txIn := range tx.MsgTx().TxIn {
		...
		if _, exists := mp.orphansByPrev[originTxHash]; !exists {
			mp.orphansByPrev[originTxHash] =
				make(map[chainhash.Hash]*cdrutil.Tx)
		}
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAddOrphan(tx *cdrutil.Tx) error {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeTransaction(tx *cdrutil.Tx, removeRedeemers bool) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveDoubleSpends(tx *cdrutil.Tx) {
	// Protect concurrent access.
	mp.mtx.Lock()
	for _, txIn := range tx.MsgTx().TxIn {
...
func (mp *TxPool) addTransaction(utxoView *blockchain.UtxoViewpoint,
	tx *cdrutil.Tx, txType stake.TxType, height int64, fee int64) {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) checkPoolDoubleSpend(tx *cdrutil.Tx, txType stake.TxType) error {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) fetchInputUtxos(tx *cdrutil.Tx) (*blockchain.UtxoViewpoint, error) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) FetchTransaction(txHash *chainhash.Hash, includeRecentBlock bool) (*cdrutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.RLock()
	...
	// For commanderu, the latest block is considered "unconfirmed"
...
// It should also set the cdrutil tree type for the tx as well.
func (mp *TxPool) maybeAcceptTransaction(tx *cdrutil.Tx, isNew, rateLimit, allowHighFees bool) ([]*chainhash.Hash, error) {
	msgTx := tx.MsgTx()
	txHash := tx.Hash()
...
//
// This function is safe for concurrent access.
func (mp *TxPool) MaybeAcceptTransaction(tx *cdrutil.Tx, isNew, rateLimit bool) ([]*chainhash.Hash, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) processOrphans(hash *chainhash.Hash) []*cdrutil.Tx {
	var acceptedTxns []*cdrutil.Tx
...
//
// This function is safe for concurrent access.
func (mp *TxPool) ProcessTransaction(tx *cdrutil.Tx, allowOrphan, rateLimit, allowHighFees bool) ([]*cdrutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
	defer mp.mtx.Unlock()
	var err error
	...
	if len(missingParents) == 0 {
		...
		acceptedTxs := make([]*cdrutil.Tx, len(newTxs)+1)
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RawMempoolVerbose(filterType *stake.TxType) map[string]*cdrjson.GetRawMempoolVerboseResult {
	mp.mtx.RLock()
	defer mp.mtx.RUnlock()

	result := make(map[string]*cdrjson.GetRawMempoolVerboseResult,
		len(mp.pool))
	bestHeight := mp.cfg.BestHeight()
	...
	for _, desc := range mp.pool {
		...
		mpd := &cdrjson.GetRawMempoolVerboseResult{
			Size:             int32(tx.MsgTx().SerializeSize()),
			Fee:              cdrutil.Amount(desc.Fee).ToCoin(),
...
func New(cfg *Config) *TxPool {
	return &TxPool{
		...
		orphans:       make(map[chainhash.Hash]*cdrutil.Tx),
		orphansByPrev: make(map[chainhash.Hash]map[chainhash.Hash]*cdrutil.Tx),
		outpoints:     make(map[wire.OutPoint]*cdrutil.Tx),
