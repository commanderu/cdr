...
import (
	...
	"github.com/commanderu/cdrd/cdrec/secp256k1"
...
type fakeChain struct {
	...
	blocks        map[chainhash.Hash]*cdrutil.Block
...
func (s *fakeChain) FetchUtxoView(tx *cdrutil.Tx, treeValid bool) (*blockchain.UtxoViewpoint, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *fakeChain) BlockByHash(hash *chainhash.Hash) (*cdrutil.Block, error) {
	s.RLock()
...
func (s *fakeChain) AddBlock(block *cdrutil.Block) {
	s.Lock()
...
func (s *fakeChain) CalcSequenceLock(tx *cdrutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
...
// transactions.
func txOutToSpendableOut(tx *cdrutil.Tx, outputNum uint32) spendableOutput {
	return spendableOutput{
		outPoint: wire.OutPoint{Hash: *tx.Hash(), Index: outputNum},
		amount:   cdrutil.Amount(tx.MsgTx().TxOut[outputNum].Value),
	}
}
...
// version 2 blocks.
func (p *poolHarness) CreateCoinbaseTx(blockHeight int64, numOutputs uint32) (*cdrutil.Tx, error) {
...
func (p *poolHarness) CreateSignedTx(inputs []spendableOutput, numOutputs uint32) (*cdrutil.Tx, error) {
	// Calculate the total input amount and split it amongst the requested
	// number of outputs.
	var totalInput cdrutil.Amount
...
func (p *poolHarness) CreateTxChain(firstOutput spendableOutput, numTxns uint32) ([]*cdrutil.Tx, error) {
	txChain := make([]*cdrutil.Tx, 0, numTxns)
	...
	for i := uint32(0); i < numTxns; i++ {
		...
		txChain = append(txChain, cdrutil.NewTx(tx))
...
func newPoolHarness(chainParams *chaincfg.Params) (*poolHarness, []spendableOutput, error) {
	...
	payPubKeyAddr, err := cdrutil.NewAddressSecpPubKey(pubKeyBytes,
		chainParams)
	if err != nil {
		return nil, nil, err
	}
	...
	chain := &fakeChain{
		...
		blocks:      make(map[chainhash.Hash]*cdrutil.Block),
