...
import (
	...
	"github.com/commanderu/cdrd/blockchain"
...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *cdrutil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *cdrutil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *cdrutil.Tx, isNew bool) {
...
func (f *wsClientFilter) addAddress(a cdrutil.Address) {
	switch a := a.(type) {
	case *cdrutil.AddressPubKeyHash:
		f.pubKeyHashes[*a.Hash160()] = struct{}{}
		return
	case *cdrutil.AddressScriptHash:
...
func (f *wsClientFilter) existsAddress(a cdrutil.Address) bool {
	switch a := a.(type) {
	case *cdrutil.AddressPubKeyHash:
...
// Notification types
type notificationBlockConnected cdrutil.Block
type notificationBlockDisconnected cdrutil.Block
...
func (m *wsNotificationManager) subscribedClients(tx *cdrutil.Tx, clients map[chan struct{}]*wsClient) map[chan struct{}]struct{} {
...
func (m *wsNotificationManager) notifyBlockConnected(clients map[chan struct{}]*wsClient, block *cdrutil.Block) {
	...
	ntfn := cdrjson.BlockConnectedNtfn{
	...
	for quitChan, client := range clients {
		...
		marshalledJSON, err := cdrjson.MarshalCmd("1.0", nil, &ntfn)
		if err != nil {
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *cdrutil.Block) {
	...
	ntfn := cdrjson.BlockDisconnectedNtfn{
		Header: hex.EncodeToString(headerBytes),
	}
	marshalledJSON, err := cdrjson.MarshalCmd("1.0", nil, &ntfn)
	if err != nil {
...
func (m *wsNotificationManager) notifyReorganization(clients map[chan struct{}]*wsClient, rd *blockchain.ReorganizationNtfnsData) {
	...
	// Notify interested websocket clients about the disconnected block.
	ntfn := cdrjson.NewReorganizationNtfn(rd.OldHash.String(),
		int32(rd.OldHeight),
		rd.NewHash.String(),
		int32(rd.NewHeight))
	marshalledJSON, err := cdrjson.MarshalCmd("1.0", nil, ntfn)
...
func (*wsNotificationManager) notifyWinningTickets(
	...
	ntfn := cdrjson.NewWinningTicketsNtfn(wtnd.BlockHash.String(),
...
func (*wsNotificationManager) notifySpentAndMissedTickets(clients map[chan struct{}]*wsClient, tnd *blockchain.TicketNotificationsData) {
	...
	// Notify interested websocket clients about the connected block.
	ntfn := cdrjson.NewSpentAndMissedTicketsNtfn(tnd.Hash.String(),
...
func (*wsNotificationManager) notifyNewTickets(clients map[chan struct{}]*wsClient, tnd *blockchain.TicketNotificationsData) {
	...
	// Notify interested websocket clients about the connected block.
	ntfn := cdrjson.NewNewTicketsNtfn(tnd.Hash.String(), int32(tnd.Height),
...
func (*wsNotificationManager) notifyStakeDifficulty(clients map[chan struct{}]*wsClient, sdnd *StakeDifficultyNtfnData) {
	// Notify interested websocket clients about the connected block.
	ntfn := cdrjson.NewStakeDifficultyNtfn(sdnd.BlockHash.String(),
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *cdrutil.Tx) {
	...
	ntfn := cdrjson.NewTxAcceptedNtfn(txHashStr,
		cdrutil.Amount(amount).ToCoin())
	marshalledJSON, err := cdrjson.MarshalCmd("1.0", nil, ntfn)
	if err != nil {
	...
	var verboseNtfn *cdrjson.TxAcceptedVerboseNtfn
	...
	for _, wsc := range clients {
		if wsc.verboseTxUpdates {
			...
			verboseNtfn = cdrjson.NewTxAcceptedVerboseNtfn(*rawTx)
			marshalledJSONVerbose, err = cdrjson.MarshalCmd("1.0", nil,
				verboseNtfn)
			if err != nil {
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *cdrutil.Tx,
	...
	if len(clientsToNotify) != 0 {
		n := cdrjson.NewRelevantTxAcceptedNtfn(txHexString(msgTx))
		marshalled, err := cdrjson.MarshalCmd("1.0", nil, n)
		if err != nil {
	...
	for {
		...
		if !batchedRequest {
			...
			if req.Method == "" || req.Params == nil {
				jsonErr := &cdrjson.RPCError{
					Code:    cdrjson.ErrRPCInvalidRequest.Code,
			...
			switch authCmd, ok := cmd.cmd.(*cdrjson.AuthenticateCmd); {
			...
			if !c.isAdmin {
				if _, ok := rpcLimited[req.Method]; !ok {
					jsonErr := &cdrjson.RPCError{
						Code:    cdrjson.ErrRPCInvalidParams.Code,
		...
		if batchedRequest {
			...
			if err != nil {
				...
				reply, err = cdrjson.MarshalResponse("2.0", nil, nil, jsonErr)
				if err != nil {
...
func handleWebsocketHelp(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*cdrjson.HelpCmd)
	if !ok {
		return nil, cdrjson.ErrRPCInternal
	}
...
func handleLoadTxFilter(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*cdrjson.LoadTxFilterCmd)
...
func handleSession(wsc *wsClient, icmd interface{}) (interface{}, error) {
	return &cdrjson.SessionResult{SessionID: wsc.sessionID}, nil
}
...
func handleNotifyNewTransactions(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*cdrjson.NotifyNewTransactionsCmd)
	if !ok {
		return nil, cdrjson.ErrRPCInternal
	}
...
// string slice.
func rescanBlock(filter *wsClientFilter, block *cdrutil.Block) []string {
...
func handleRescan(wsc *wsClient, icmd interface{}) (interface{}, error) {
	...
	if filter == nil {
		return nil, &cdrjson.RPCError{
			Code:    cdrjson.ErrRPCMisc,
			Message: "Transaction filter must be loaded before rescanning",
		}
	}

	blockHashes, err := cdrjson.DecodeConcatenatedHashes(cmd.BlockHashes)
	if err != nil {
		return nil, err
	}

	discoveredData := make([]cdrjson.RescannedBlock, 0, len(blockHashes))
	...
	for i := range blockHashes {
		...
		if err != nil {
			return nil, &cdrjson.RPCError{
				Code:    cdrjson.ErrRPCBlockNotFound,
		...
		if len(transactions) != 0 {
			discoveredData = append(discoveredData, cdrjson.RescannedBlock{
	...
	return &cdrjson.RescanResult{DiscoveredData: discoveredData}, nil
}
