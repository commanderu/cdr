...
const (
	// svcName is the name of cdrd service.
	svcName = "cdrdsvc"
	...
	svcDisplayName = "cdrd Service"

	// svcDesc is the description of the service.
	svcDesc = "Downloads and stays synchronized with the commanderu block " +
...
// logServiceStartOfDay logs information about cdrd when the main server has
...
// cdrdService houses the main service handler which handles all service
// updates and launching cdrdMain.
type cdrdService struct{}
...
// long-running cdrdMain (which is the real meat of cdrd), handles service
// change requests, and notifies the service control manager of changes.
func (s *cdrdService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start cdrdMain in a separate goroutine so the service can start
...
// installService attempts to install the cdrd service.  Typically this should
...
// removeService attempts to uninstall the cdrd service.  Typically this should
...
// startService attempts to start the cdrd service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &cdrdService{})
	if err != nil {
