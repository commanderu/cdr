...
// FileContents is a string containing the commented example config for cdrd.
...
; The default is ~/.cdrd/data on POSIX OSes, $LOCALAPPDATA/cdrd/data on Windows,
; ~/Library/Application Support/cdrd/data on macOS, and $homed/cdrd/data on
; Plan9.  Environment variables are expanded so they may be used.  NOTE: Windows
; environment variables are typically %VARIABLE%, but they must be accessed with
; $VARIABLE here.
; datadir=~/.cdrd/data                            ; Unix
; datadir=$LOCALAPPDATA/cdrd/data                 ; Windows
; datadir=~/Library/Application Support/cdrd/data ; macOS
...
; line.  cdrd will not contact 3rd-party sites to obtain external ip addresses.
...
; with, but the behavior is slightly different.  By default, cdrd will query DNS
...
; Disable DNS seeding for peers.  By default, when cdrd starts, it will use
...
; which is used to control and query information from a running cdrd process.
;
...
; All ipv6 interfaces on default port:
;   rpclisten=::
; All interfaces on port 37460:
;   rpclisten=:37460
; All ipv4 interfaces on port 37460:
;   rpclisten=0.0.0.0:37460
; All ipv6 interfaces on port 37460:
;   rpclisten=[::]:37460
; Only ipv4 localhost on port 37460:
;   rpclisten=127.0.0.1:37460
; Only ipv6 localhost on port 37460:
;   rpclisten=[::1]:37460
...
; log level for individual subsystems.  Use cdrd --debuglevel=show to list
