...
import (
	...
	"github.com/commanderu/cdrd/addrmgr"
	...
	"github.com/commanderu/cdrd/connmgr"
	...
	"github.com/commanderu/cdrd/peer"
...
var (
	...
	// ourselves to other commanderu peers.
	userAgentName = "cdrd"
...
// broadcastMsg provides the ability to house a commanderu message to be broadcast
...
// server provides a commanderu server for handling communications to and from
// commanderu peers.
type server struct {
...
func (sp *serverPeer) OnTx(p *peer.Peer, msg *wire.MsgTx) {
	...
	// Convert the raw MsgTx to a cdrutil.Tx which provides some convenience
	// methods and things such as hash caching.
	tx := cdrutil.NewTx(msg)
...
func (sp *serverPeer) OnBlock(p *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a cdrutil.Block which provides some
	// convenience methods and things such as hash caching.
	block := cdrutil.NewBlockFromBlockAndBytes(msg, buf)
...
func (s *server) AnnounceNewTransactions(newTxs []*cdrutil.Tx) {
...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, defaultRequiredServices, cdrdLookup, func(addrs []*wire.NetAddress) {
...
// AddedNodeInfo returns an array of cdrjson.GetAddedNodeInfoResult structures
...
// newServer returns a new cdrd server configured to listen on addr for the
// commanderu network type specified by chainParams.  Use start to begin accepting
...
func newServer(listenAddrs []string, db database.DB, chainParams *chaincfg.Params, interrupt <-chan struct{}) (*server, error) {
	...
	amgr := addrmgr.New(cfg.DataDir, cdrdLookup)
...
func addrStringToNetAddr(addr string) (net.Addr, error) {
	...
	// The cdrdLookup function will transparently handle performing the
	// lookup over Tor if necessary.
	ips, err := cdrdLookup(host)
	if err != nil {
		return nil, err
	}
	if len(ips) == 0 {
