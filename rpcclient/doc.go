...
with a commanderu RPC server that uses a mostly btcd/bitcoin core
style commanderu JSON-RPC API.  This client has been tested with cdrd
(https://github.com/commanderu/cdrd) and cdrwallet
(https://github.com/commanderu/cdrwallet).

In addition to the compatible standard HTTP POST JSON-RPC API, cdrd and
cdrwallet provide a websocket interface that is more efficient than the standard
...
cdrd or cdrwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by cdrd and
cdrwallet only uses a single connection that remains open and allows
...
All notifications provided by cdrd require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by cdrd (and
cdrwallet passthrough).  Therefore if you call one of these commands against
...
Also, it is important to realize that cdrd intentionally separates the wallet
functionality into a separate process named cdrwallet.  This means if you are
connected to the cdrd RPC server directly, only the RPCs which are related to
chain services will be available.  Depending on your application, you might only
need chain-related RPCs.  In contrast, cdrwallet provides pass through treatment
...
detected by type asserting the error in a *cdrjson.RPCError.  For example, to
detect if a command is unimplemented by the remote RPC server:

  amount, err := client.GetBalance("")
  if err != nil {
  	if jerr, ok := err.(*cdrjson.RPCError); ok {
  		switch jerr.Code {
  		case cdrjson.ErrRPCUnimplemented:
...
The following full-blown client examples are in the examples directory:
 ...
 - cdrdwebsockets
   ...
   block count
 - cdrwalletwebsockets
   Connects to a cdrwallet RPC server using TLS-secured websockets, registers
