...
type NotificationHandlers struct {
	...
	// made to register for the notification and the function is non-nil.
	OnTxAccepted func(hash *chainhash.Hash, amount cdrutil.Amount)
	...
	// such as cdrwallet.
	OnAccountBalance func(account string, balance cdrutil.Amount, confirmed bool)
	...
	// This will only be available when client is connected to a wallet
	// server such as cdrwallet.
	OnTicketsPurchased func(TxHash *chainhash.Hash, amount cdrutil.Amount)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	switch ntfn.Method {
	// OnBlockConnected
	case cdrjson.BlockConnectedNtfnMethod:
	...
	// OnBlockDisconnected
	case cdrjson.BlockDisconnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case cdrjson.RelevantTxAcceptedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case cdrjson.ReorganizationNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnWinningTickets
	case cdrjson.WinningTicketsNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case cdrjson.SpentAndMissedTicketsNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnNewTickets
	case cdrjson.NewTicketsNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnStakeDifficulty
	case cdrjson.StakeDifficultyNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAccepted
	case cdrjson.TxAcceptedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAcceptedVerbose
	case cdrjson.TxAcceptedVerboseNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnBtcdConnected
	case cdrjson.BtcdConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
		...
		if err != nil {
			log.Warnf("Received invalid cdrd connected "+
				"notification: %v", err)
			return
		}
	...
	// OnAccountBalance
	case cdrjson.AccountBalanceNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTicketPurchased:
	case cdrjson.TicketPurchasedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnVotesCreated:
	case cdrjson.VoteCreatedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case cdrjson.RevocationCreatedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnWalletLockState
	case cdrjson.WalletLockStateNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
...
func parseTxAcceptedNtfnParams(params []json.RawMessage) (*chainhash.Hash,
	...
	// Bounds check amount.
	amt, err := cdrutil.NewAmount(famt)
	if err != nil {
		return nil, 0, err
	}
...
func parseTxAcceptedVerboseNtfnParams(params []json.RawMessage) (*cdrjson.TxRawResult,
	error) {
...
// parseBtcdConnectedNtfnParams parses out the connection status of cdrd
// and cdrwallet from the parameters of a btcdconnected notification.
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance cdrutil.Amount, confirmed bool, err error) {
	...
	// Bounds check amount.
	bal, err := cdrutil.NewAmount(fbal)
	if err != nil {
		return "", 0, false, err
	}
...
func parseTicketPurchasedNtfnParams(params []json.RawMessage) (txHash *chainhash.Hash,
	amount cdrutil.Amount, err error) {
	...
	return thHash, cdrutil.Amount(amt), nil
}
...
//
// NOTE: This is a cdrd extension and requires a websocket connection.
func (c *Client) NotifyBlocksAsync() FutureNotifyBlocksResult {
	...
	cmd := cdrjson.NewNotifyBlocksCmd()
...
func (c *Client) NotifyWinningTicketsAsync() FutureNotifyWinningTicketsResult {
	...
	cmd := cdrjson.NewNotifyWinningTicketsCmd()
...
func (c *Client) NotifySpentAndMissedTicketsAsync() FutureNotifySpentAndMissedTicketsResult {
	...
	cmd := cdrjson.NewNotifySpentAndMissedTicketsCmd()
...
func (c *Client) NotifyNewTicketsAsync() FutureNotifyNewTicketsResult {
	...
	cmd := cdrjson.NewNotifyNewTicketsCmd()
...
func (c *Client) NotifyStakeDifficultyAsync() FutureNotifyStakeDifficultyResult {
	...
	cmd := cdrjson.NewNotifyStakeDifficultyCmd()
...
func (c *Client) NotifyNewTransactionsAsync(verbose bool) FutureNotifyNewTransactionsResult {
	...
	cmd := cdrjson.NewNotifyNewTransactionsCmd(&verbose)
	return c.sendCmd(cmd)
}
...
//
// NOTE: This is a cdrd extension and requires a websocket connection.
func (c *Client) LoadTxFilterAsync(reload bool, addresses []cdrutil.Address,
	...
	outPointObjects := make([]cdrjson.OutPoint, len(outPoints))
	for i := range outPoints {
		outPointObjects[i] = cdrjson.OutPoint{
	...
	cmd := cdrjson.NewLoadTxFilterCmd(reload, addrStrs, outPointObjects)
	return c.sendCmd(cmd)
}
...
//
// NOTE: This is a cdrd extension and requires a websocket connection.
func (c *Client) LoadTxFilter(reload bool, addresses []cdrutil.Address, outPoints []wire.OutPoint) error {
