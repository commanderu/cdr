...
// Client represents a commanderu RPC client which allows easy access to the
// various RPC methods available on a commanderu RPC server.  Each of the wrapper
...
func (c *Client) trackRegisteredNtfns(cmd interface{}) {
	...
	case *cdrjson.NotifyWinningTicketsCmd:
		c.ntfnState.notifyWinningTickets = true

	case *cdrjson.NotifySpentAndMissedTicketsCmd:
		c.ntfnState.notifySpentAndMissedTickets = true

	case *cdrjson.NotifyNewTicketsCmd:
		c.ntfnState.notifyNewTickets = true

	case *cdrjson.NotifyStakeDifficultyCmd:
		c.ntfnState.notifyStakeDifficulty = true

	case *cdrjson.NotifyBlocksCmd:
		c.ntfnState.notifyBlocks = true

	case *cdrjson.NotifyNewTransactionsCmd:
...
type (
	...
	rawResponse struct {
		...
		Error  *cdrjson.RPCError `json:"error"`
	}
)
...
// returning an unmarshaled cdrjson.RPCError (or an unmarshaling error) if so.
...
func (c *Client) sendCmd(cmd interface{}) chan *response {
	...
	method, err := cdrjson.CmdMethod(cmd)
	if err != nil {
	...
	// Marshal the command.
	id := c.NextID()
	marshalledJSON, err := cdrjson.MarshalCmd("1.0", id, cmd)
	if err != nil {
		return newFutureError(err)
	}
