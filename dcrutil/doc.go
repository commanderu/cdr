...
A Block defines a commanderu block that provides easier and more efficient
...
A Tx defines a commanderu transaction that provides more efficient manipulation of
...
The Address interface provides an abstraction for a commanderu address.  While the
most common type is a pay-to-pubkey-hash, commanderu already supports others and
...
To decode/encode an address:
	...
	addr, err := cdrutil.DecodeAddress(addrString, defaultNet)
	if err != nil {
		fmt.Println(err)
		return
	}
