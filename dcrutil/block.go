...
// Tx returns a wrapped transaction (cdrutil.Tx) for the transaction at the
...
// STx returns a wrapped transaction (cdrutil.Tx) for the stake transaction at
...
// Transactions returns a slice of wrapped transactions (cdrutil.Tx) for all
...
// instead provides easy access to wrapped versions (cdrutil.Tx) of them.
...
// STransactions returns a slice of wrapped stake transactions (cdrutil.Tx) for all
// stake transactions in the Block.  This is nearly equivalent to accessing the raw
// transactions (cdrwire.MsgTx) in the underlying wire.MsgBlock, however it
