...
import (
	...
	"github.com/commanderu/cdrd/mempool"
...
func containsTxIns(txs []*cdrutil.Tx, tx *cdrutil.Tx) bool {
...
// can not be found.
func txIndexFromTxList(hash chainhash.Hash, list []*cdrutil.Tx) int {
	for i, tx := range list {
		h := tx.Hash()
		if hash == *h {
			return i
...
func UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int64, extraNonce uint64) error {
	...
	// TODO(davec): A cdrutil.Block should use saved in the state to avoid
	// recalculating all of the other transaction hashes.
	// block.Transactions[0].InvalidateCache()

	// Recalculate the merkle root with the updated extra nonce.
	block := cdrutil.NewBlockDeepCopyCoinbase(msgBlock)
...
func createCoinbaseTx(subsidyCache *blockchain.SubsidyCache, coinbaseScript []byte, opReturnPkScript []byte, nextBlockHeight int64, addr cdrutil.Address, voters uint16, params *chaincfg.Params) (*cdrutil.Tx, error) {
	tx := wire.NewMsgTx()
	...
	if nextBlockHeight == 1 && len(params.BlockOneLedger) != 0 {
		...
		addrs := make([]cdrutil.Address, len(params.BlockOneLedger))
		for i, payout := range params.BlockOneLedger {
			addr, err := cdrutil.DecodeAddress(payout.Address)
			if err != nil {
				return nil, err
			}
			addrs[i] = addr
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *cdrutil.Tx, height int64) error {
	for _, txIn := range tx.MsgTx().TxIn {
...
func logSkippedDeps(tx *cdrutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
func maybeInsertStakeTx(bm *blockManager, stx *cdrutil.Tx, treeValid bool) bool {
	missingInput := false
...
func deepCopyBlockTemplate(blockTemplate *BlockTemplate) *BlockTemplate {
	...
	coinbaseCopy :=
		cdrutil.NewTxDeep(blockTemplate.Block.Transactions[0])
...
func handleTooFewVoters(subsidyCache *blockchain.SubsidyCache, nextHeight int64, miningAddress cdrutil.Address, bm *blockManager) (*BlockTemplate, error) {
	...
	if nextHeight >= stakeValidationHeight {
		if bm.AggressiveMining {
			if curTemplate != nil {
				...
				block := cdrutil.NewBlockDeepCopyCoinbase(cptCopy.Block)
			...
			if curTemplate == nil {
				...
				btBlockTemp := cdrutil.NewBlockDeepCopyCoinbase(btMsgBlock)
				merkles :=
				...
				// Make sure the block validates.
				btBlock := cdrutil.NewBlockDeepCopyCoinbase(btMsgBlock)
...
func NewBlockTemplate(policy *mining.Policy, server *server, payToAddress cdrutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*cdrutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		// Consider caching this in the mempool in the future. - commanderu
	...
	blockTxnsStake := make([]*cdrutil.Tx, 0, len(blockTxns))
	...
	for _, tx := range blockTxns {
		...
		if stake.IsSSGen(msgTx) {
			txCopy := cdrutil.NewTxDeepTxIns(msgTx)
	...
	if nextBlockHeight < stakeValidationHeight {
		...
		for _, vb := range voteBitsVoters {
			if cdrutil.IsFlagSet16(vb, cdrutil.BlockValid) {
				voteYea++
			}
			totalVotes++
		}
		...
		if votebits == uint16(0x0000) {
			...
			// the previous block.
			// commanderu WARNING: The ideal behaviour should also be that we re-add
			...
			tempBlockTxns := make([]*cdrutil.Tx, 0, len(sourceTxns))
			for _, tx := range blockTxns {
				if tx.Tree() == wire.TxTreeRegular {
					...
					if isValid {
						txCopy := cdrutil.NewTxDeepTxIns(tx.MsgTx())
	...
	// commanderu: We need to move this downwards because of the requirements
	...
	blockTxnsRegular := make([]*cdrutil.Tx, 0, len(blockTxns)+1)
	...
	minrLog.Debugf("Created new block template (%d transactions, %d "+
		...
		cdrutil.Amount(msgBlock.Header.SBits).ToCoin())
