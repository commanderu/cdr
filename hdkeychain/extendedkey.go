...
func (k *ExtendedKey) Child(i uint32) (*ExtendedKey, error) {
	...
	parentFP := cdrutil.Hash160(k.pubKeyBytes())[:4]
...
// ECPubKey converts the extended key to a cdrec public key and returns it.
func (k *ExtendedKey) ECPubKey() (chainec.PublicKey, error) {
	return chainec.Secp256k1.ParsePubKey(k.pubKeyBytes())
}

// ECPrivKey converts the extended key to a cdrec private key and returns it.
...
// Address converts the extended key to a standard commanderu pay-to-pubkey-hash
// address for the passed network.
func (k *ExtendedKey) Address(net *chaincfg.Params) (*cdrutil.AddressPubKeyHash, error) {
	pkHash := cdrutil.Hash160(k.pubKeyBytes())
	return cdrutil.NewAddressPubKeyHash(pkHash, net, chainec.ECTypeSecp256k1)
}
