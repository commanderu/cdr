...
// processes to communicate with and and manage the lifetime of a cdrd child
...
// message, shutdown begins.  This prevents cdrd from continuing to run
...
func serviceControlPipeRx(fd uintptr) {
	...
	for {
		...
		if err != nil {
			cdrdLog.Errorf("Failed to read from pipe: %v", err)
			break
		}
	}
...
// for a child cdrd process to communicate with a parent process without the
...
func serviceControlPipeTx(fd uintptr) {
	...
	cdrdLog.Errorf("Failed to write to pipe: %v", err)
}
