...
import (
	...
	"github.com/commanderu/cdrd/chaincfg/chainec"
...
// newAddressPubKey returns a new cdrutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) cdrutil.Address {
	pubkey, err := chainec.Secp256k1.ParsePubKey(serializedPubKey)
	if err != nil {
		panic("invalid public key in test source")
	}
	addr, err := cdrutil.NewAddressSecpPubKeyCompressed(pubkey,
...
// newAddressPubKeyHash returns a new cdrutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) cdrutil.Address {
	addr, err := cdrutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams,
...
// newAddressScriptHash returns a new cdrutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) cdrutil.Address {
	addr, err := cdrutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the cdrutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the cdrutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := cdrutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := cdrutil.NewAddressScriptHashFromHash(hexToBytes("e8c30"+
	...
	p2pkCompressedMain, err := cdrutil.NewAddressSecpPubKey(hexToBytes("02192d7"+
	...
	p2pkCompressed2Main, err := cdrutil.NewAddressSecpPubKey(hexToBytes("03b0b"+
	...
	tests := []struct {
		...
		// pay-to-pubkey address on mainnet. for commanderu this would
		// be uncompressed, but standard for commanderu is 33 byte
		...
		{(*cdrutil.AddressPubKeyHash)(nil), "", ErrUnsupportedAddress},
		{(*cdrutil.AddressScriptHash)(nil), "", ErrUnsupportedAddress},
		{(*cdrutil.AddressSecpPubKey)(nil), "", ErrUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	//  mainnet p2pk 13CG6SJ3yHUXo4Cr2RY4THLLJrNFuG3gUg
	p2pkCompressedMain, err := cdrutil.NewAddressSecpPubKey(hexToBytes("02192d"+
	...
	tests := []struct {
		keys      []*cdrutil.AddressSecpPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*cdrutil.AddressSecpPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
		...
		{
			// By default compressed pubkeys are used in commanderu.
			[]*cdrutil.AddressSecpPubKey{
				p2pkUncompressedMain.(*cdrutil.AddressSecpPubKey),
			},
			1,
