...
// valid SStxs.
func PayToSStx(addr cdrutil.Address) ([]byte, error) {
	if addr == nil {
...
// valid SStxs.
func PayToSStxChange(addr cdrutil.Address) ([]byte, error) {
	if addr == nil {
		return nil, ErrUnsupportedAddress
	}
...
// valid SSGen txs.
func PayToSSGen(addr cdrutil.Address) ([]byte, error) {
	if addr == nil {
		return nil, ErrUnsupportedAddress
	}
...
// valid SSRtx.
func PayToSSRtx(addr cdrutil.Address) ([]byte, error) {
	if addr == nil {
		return nil, ErrUnsupportedAddress
	}
...
// an SStx.
func GenerateSStxAddrPush(addr cdrutil.Address, amount cdrutil.Amount,
...
// specified address.
func PayToAddrScript(addr cdrutil.Address) ([]byte, error) {
	switch addr := addr.(type) {
	case *cdrutil.AddressPubKeyHash:
		if addr == nil {
			return nil, ErrUnsupportedAddress
...
func MultiSigScript(pubkeys []*cdrutil.AddressSecpPubKey, nrequired int) ([]byte,
	error) {
...
func ExtractPkScriptAddrs(version uint16, pkScript []byte,
	chainParams *chaincfg.Params) (ScriptClass, []cdrutil.Address, int, error) {
	if version != DefaultScriptVersion {
	...
	case PubKeyHashTy:
		...
		requiredSigs = 1
		addr, err := cdrutil.NewAddressPubKeyHash(pops[2].data,
	...
	case PubKeyTy:
		...
		if err == nil {
			addr, err := cdrutil.NewAddressSecpPubKeyCompressed(pk, chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
		}
	...
	case PubkeyAltTy:
		...
		switch suite {
		case chainec.ECTypeEdwards:
			addr, err = cdrutil.NewAddressEdwardsPubKey(pops[0].data,
				chainParams)
		case chainec.ECTypeSecSchnorr:
			addr, err = cdrutil.NewAddressSecSchnorrPubKey(pops[0].data,
				chainParams)
		}
		if err == nil {
			addrs = append(addrs, addr)
	...
	case StakeSubmissionTy:
		...
		var localAddrs []cdrutil.Address
	...
	case ScriptHashTy:
		...
		requiredSigs = 1
		addr, err := cdrutil.NewAddressScriptHashFromHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case MultiSigTy:
		...
		addrs = make([]cdrutil.Address, 0, numPubKeys)
...
//
// NOTE: Atomic swaps are not considered standard script types by the cdrd
