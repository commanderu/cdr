...
func mkGetKey(keys map[string]addressToKey) KeyDB {
	if keys == nil {
		return KeyClosure(func(addr cdrutil.Address) (chainec.PrivateKey,
			bool, error) {
...
func mkGetScript(scripts map[string][]byte) ScriptDB {
	if scripts == nil {
		return ScriptClosure(func(addr cdrutil.Address) (
			[]byte, error) {
			return nil, errors.New("nope 3")
		})
	}
	return ScriptClosure(func(addr cdrutil.Address) ([]byte,
		error) {
...
func TestSignTxOutput(t *testing.T) {
	...
	for _, hashType := range hashTypes {
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				address, err := cdrutil.NewAddressPubKeyHash(
					cdrutil.Hash160(pkBytes), testingParams,
					suite)
	...
	for _, hashType := range hashTypes {
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				// for more details
				address, err := cdrutil.NewAddressSecpPubKeyCompressed(pk,
					testingParams)
				if err != nil {
					t.Errorf("failed to make address for %s: %v",
						msg, err)
		...
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				case chainec.ECTypeEdwards:
					...
					key, pk = chainec.Edwards.PrivKeyFromBytes(keyDB)
					pkBytes = pk.SerializeUncompressed()
					address, err = cdrutil.NewAddressEdwardsPubKey(pkBytes,
						testingParams)
					if err != nil {
						t.Errorf("failed to make address for %s: %v",
							msg, err)
				...
				case chainec.ECTypeSecSchnorr:
					...
					key, pk = chainec.SecSchnorr.PrivKeyFromBytes(keyDB)
					pkBytes = pk.Serialize()
					address, err = cdrutil.NewAddressSecSchnorrPubKey(pkBytes,
						testingParams)
					if err != nil {
						t.Errorf("failed to make address for %s: %v",
							msg, err)
	...
	for _, hashType := range hashTypes {
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				address, err := cdrutil.NewAddressPubKeyHash(
					cdrutil.Hash160(pkBytes), testingParams, suite)
				if err != nil {
					t.Errorf("failed to make address for %s: %v",
						msg, err)
					break
				...
				scriptAddr, err := cdrutil.NewAddressScriptHash(
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			address1, err := cdrutil.NewAddressSecpPubKeyCompressed(pk1,
				testingParams)
			if err != nil {
				t.Errorf("failed to make address for %s: %v",
					msg, err)
			...
			address2, err := cdrutil.NewAddressSecpPubKeyCompressed(pk2,
				testingParams)
			if err != nil {
			...
			pkScript, err := MultiSigScript(
				[]*cdrutil.AddressSecpPubKey{address1, address2},
				2)
			if err != nil {
				t.Errorf("failed to make pkscript "+
					"for %s: %v", msg, err)
...
var (
	...
	thisAddressUnc, _ = cdrutil.NewAddressPubKeyHash(
		cdrutil.Hash160(thisPubKey.SerializeUncompressed()),
		testingParams, chainec.ECTypeSecp256k1)
	uncompressedPkScript, _ = PayToAddrScript(thisAddressUnc)
	thisAddressCom, _       = cdrutil.NewAddressPubKeyHash(
		cdrutil.Hash160(thisPubKey.SerializeCompressed()),
		testingParams, chainec.ECTypeSecp256k1)
