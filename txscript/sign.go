...
func signMultiSig(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType,
	addresses []cdrutil.Address, nRequired int, kdb KeyDB) ([]byte, bool) {
	// No need to add dummy in commanderu.
...
func handleStakeOutSign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	addresses []cdrutil.Address, class ScriptClass, subClass ScriptClass,
	nrequired int) ([]byte, ScriptClass, []cdrutil.Address, int, error) {
...
func sign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	ScriptClass, []cdrutil.Address, int, error) {
...
func mergeScripts(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	pkScript []byte, class ScriptClass, addresses []cdrutil.Address,
...
func mergeMultiSig(tx *wire.MsgTx, idx int, addresses []cdrutil.Address,
...
sigLoop:
	for _, sig := range possibleSigs {
		...
		if err != nil {
			// commanderu -- is this the right handling for SIGHASH_SINGLE error ?
		...
		for _, addr := range addresses {
			...
			// bad input.
			pkaddr := addr.(*cdrutil.AddressSecpPubKey)
...
type KeyDB interface {
	GetKey(cdrutil.Address) (chainec.PrivateKey, bool, error)
}

// KeyClosure implements KeyDB with a closure.
type KeyClosure func(cdrutil.Address) (chainec.PrivateKey, bool, error)

// GetKey implements KeyDB by returning the result of calling the closure.
func (kc KeyClosure) GetKey(address cdrutil.Address) (chainec.PrivateKey,
	bool, error) {
	return kc(address)
}
...
type ScriptDB interface {
	GetScript(cdrutil.Address) ([]byte, error)
}

// ScriptClosure implements ScriptDB with a closure.
type ScriptClosure func(cdrutil.Address) ([]byte, error)

// GetScript implements ScriptDB by returning the result of calling the closure.
func (sc ScriptClosure) GetScript(address cdrutil.Address) ([]byte, error) {
	return sc(address)
}
