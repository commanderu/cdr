...
import (
	...
	"github.com/commanderu/cdrd/hdkeychain"
	"github.com/commanderu/cdrd/rpcclient"
...
type memWallet struct {
	...
	addrs map[uint32]cdrutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]cdrutil.Address)
...
// SetRPCClient saves the passed rpc connection to cdrd as the wallet's
...
func (m *memWallet) IngestBlock(header []byte, filteredTxns [][]byte) {
	...
	txns := make([]*cdrutil.Tx, 0, len(filteredTxns))
	for _, txBytes := range filteredTxns {
		tx, err := cdrutil.NewTxFromBytes(txBytes)
		if err != nil {
			panic(err)
		}
		txns = append(txns, tx)
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash, isCoinbase bool, undo *undoEntry) {
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          cdrutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (cdrutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []cdrutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt cdrutil.Amount, feeRate cdrutil.Amount) error {
	const (
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := cdrutil.Amount(txSize * int(feeRate))
...
// in atoms-per-byte.
func (m *memWallet) SendOutputs(outputs []*wire.TxOut, feeRate cdrutil.Amount) (*chainhash.Hash, error) {
...
//
// This function is safe for concurrent access.
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut, feeRate cdrutil.Amount) (*wire.MsgTx, error) {
	m.Lock()
	defer m.Unlock()
	...
	var outputAmt cdrutil.Amount
	for _, output := range outputs {
		outputAmt += cdrutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() cdrutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance cdrutil.Amount
...
func keyToAddr(key chainec.PrivateKey, net *chaincfg.Params) (cdrutil.Address, error) {
	pubKey := chainec.Secp256k1.NewPublicKey(key.Public())
	serializedKey := pubKey.SerializeCompressed()
	pubKeyAddr, err := cdrutil.NewAddressSecpPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
