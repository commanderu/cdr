...
import (
	...
	"github.com/commanderu/cdrd/certgen"
	rpc "github.com/commanderu/cdrd/rpcclient"
)

// nodeConfig contains all the args, and data required to launch a cdrd process
...
// arguments returns an array of arguments that be used to launch the cdrd
// process.
...
// command returns the exec.Cmd which will be used to start the cdrd process.
...
// to the cdrd process that is launched via Start().
...
// as the base for the log and data directories for cdrd.
...
// start creates a new cdrd process, and writes its pid in a file reserved for
...
func (n *node) start() error {
	...
	pid, err := os.Create(filepath.Join(n.config.String(), "cdrd.pid"))
	if err != nil {
		return err
	}
...
// stop interrupts the running cdrd process process, and waits until it exits
...
// shutdown terminates the running cdrd process, and cleans up all
