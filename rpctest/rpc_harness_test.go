...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt cdrutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(cdrutil.Amount(5 * cdrutil.AtomsPerCoin))
	...
	txid = genSpend(cdrutil.Amount(5000 * cdrutil.AtomsPerCoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testJoinMempools(r *Harness, t *testing.T) {
	...
	pooledHashes, err := r.Node.GetRawMempool(cdrjson.GRMAll)
	if err != nil {
	...
	go func() {
		for {
			poolHashes, err := r.Node.GetRawMempool(cdrjson.GRMAll)
			if err != nil {
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := cdrutil.Amount(5 * 300 * cdrutil.AtomsPerCoin)
	...
	// chain.
	expectedBalance = cdrutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := cdrutil.Amount(50 * cdrutil.AtomsPerCoin)
...
func TestHarness(t *testing.T) {
	...
	expectedBalance := cdrutil.Amount(numMatureOutputs * 300 * cdrutil.AtomsPerCoin)
