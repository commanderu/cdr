...
// Harness fully encapsulates an active cdrd process to provide a unified
// platform for creating rpc driven integration tests involving cdrd. The
// active cdrd node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active cdrd process is fully
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	// Start the cdrd node itself. This spawns a new process which will be
	// managed
	...
	// wallet.
	filterAddrs := []cdrutil.Address{h.wallet.coinbaseAddr}
	if err := h.Node.LoadTxFilter(true, filterAddrs, nil); err != nil {
		return err
	}

	// Ensure cdrd properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created cdrd
...
func (h *Harness) NewAddress() (cdrutil.Address, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) ConfirmedBalance() cdrutil.Amount {
...
//
// This function is safe for concurrent access.
func (h *Harness) SendOutputs(targetOutputs []*wire.TxOut, feeRate cdrutil.Amount) (*chainhash.Hash, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) CreateTransaction(targetOutputs []*wire.TxOut, feeRate cdrutil.Amount) (*wire.MsgTx, error) {
