...
func TestEncodeConcatenatedHashes(t *testing.T) {
	// Input Hash slice. Data taken from commanderu's first three mainnet blocks.
	...
	for j := 0; j < len(hashSlice)+1; j++ {
		...
		// Encode to string
		concatenated := cdrjson.EncodeConcatenatedHashes(hashSlice[:j])
		// Verify output
...
func TestDecodeConcatenatedHashes(t *testing.T) {
	// Test data taken from commanderu's first three mainnet blocks
	...
	decodedHashes, err := cdrjson.DecodeConcatenatedHashes(concatenatedHashes)
	if err != nil {
...
func TestEncodeConcatenatedVoteBits(t *testing.T) {
	...
	encodedResults, err := cdrjson.EncodeConcatenatedVoteBits(testVbs)
	if err != nil {
	...
	_, err = cdrjson.EncodeConcatenatedVoteBits(testVbs)
	if err == nil {
...
func TestDecodeConcatenatedVoteBits(t *testing.T) {
	...
	decodedSlice, err :=
		cdrjson.DecodeConcatenatedVoteBits(encodedBytesStr)
	if err != nil {
	...
	_, err = cdrjson.DecodeConcatenatedVoteBits(encodedBytesStr)
	if err == nil {
