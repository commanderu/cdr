...
func TestIsValidIDType(t *testing.T) {
	...
	for i, test := range tests {
		if cdrjson.IsValidIDType(test.id) != test.isValid {
...
func TestMarshalResponse(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			result: nil,
			jsonErr: func() *cdrjson.RPCError {
				return cdrjson.NewRPCError(cdrjson.ErrRPCBlockNotFound, "123 not found")
			}(),
	...
	for i, test := range tests {
		_, _ = i, test
		marshalled, err := cdrjson.MarshalResponse("1.0", testID, test.result, test.jsonErr)
		if err != nil {
...
func TestMiscErrors(t *testing.T) {
	...
	// not supported.
	_, err := cdrjson.NewRequest("1.0", nil, "test", []interface{}{make(chan int)})
	if err == nil {
	...
	// supported.
	wantErr := cdrjson.Error{Code: cdrjson.ErrInvalidType}
	_, err = cdrjson.MarshalResponse("", make(chan int), nil, nil)
	if jerr, ok := err.(cdrjson.Error); !ok || jerr.Code != wantErr.Code {
	...
	// can't be marshalled.
	_, err = cdrjson.MarshalResponse("1.0", 1, make(chan int), nil)
