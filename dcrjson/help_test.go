...
func TestHelpReflectInternals(t *testing.T) {
	...
	for i, test := range tests {
		...
		key := cdrjson.TstReflectTypeToJSONType(xT, test.reflectType)
		if key != test.key {
		...
		examples, isComplex := cdrjson.TstReflectTypeToJSONExample(xT,
		...
		helpText := cdrjson.TstResultTypeHelp(xT, test.reflectType, "fdk")
		...
		isValid := cdrjson.TstIsValidResultType(test.reflectType.Kind())
...
func TestResultStructHelp(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		results := cdrjson.TstResultStructHelp(xT, test.reflectType, 0)
...
func TestHelpArgInternals(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		help := cdrjson.TstArgHelp(xT, test.reflectType, test.defaults,
			test.method)
		if help != test.help {
...
func TestMethodHelp(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		help := cdrjson.TestMethodHelp(xT, test.reflectType,
...
func TestGenerateHelpErrors(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			method: "boguscommand",
			err:    cdrjson.Error{Code: cdrjson.ErrUnregisteredMethod},
		},
		{
		...
		},
		{
			...
			err:         cdrjson.Error{Code: cdrjson.ErrMissingDescription},
		},
	}

	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		_, err := cdrjson.GenerateHelp(test.method, nil,
			test.resultTypes...)
...
func TestGenerateHelp(t *testing.T) {
	...
	help, err := cdrjson.GenerateHelp("help", descs)
	if err != nil {
