...
func TestUsageFlagStringer(t *testing.T) {
	...
	tests := []struct {
		in   cdrjson.UsageFlag
		want string
	}{
		{0, "0x0"},
		{cdrjson.UFWalletOnly, "UFWalletOnly"},
		{cdrjson.UFWebsocketOnly, "UFWebsocketOnly"},
		{cdrjson.UFNotification, "UFNotification"},
		{cdrjson.UFWalletOnly | cdrjson.UFWebsocketOnly,
			"UFWalletOnly|UFWebsocketOnly"},
		{cdrjson.UFWalletOnly | cdrjson.UFWebsocketOnly | (1 << 31),
			"UFWalletOnly|UFWebsocketOnly|0x80000000"},
	}

	// Detect additional usage flags that don't have the stringer added.
	numUsageFlags := 0
	highestUsageFlagBit := cdrjson.TstHighestUsageFlagBit
...
func TestRegisterCmdErrors(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			err: cdrjson.Error{Code: cdrjson.ErrDuplicateMethod},
		},
		{
			...
			flags: cdrjson.TstHighestUsageFlagBit,
			err:   cdrjson.Error{Code: cdrjson.ErrInvalidUsageFlags},
		},
		{
			name:   "invalid type",
			method: "registertestcmd",
		...
		},
		{
			...
			err: cdrjson.Error{Code: cdrjson.ErrEmbeddedType},
		},
		{
			...
			err: cdrjson.Error{Code: cdrjson.ErrUnexportedField},
		},
		{
			...
			err: cdrjson.Error{Code: cdrjson.ErrUnsupportedFieldType},
		},
		{
		...
		},
		{
			...
			err: cdrjson.Error{Code: cdrjson.ErrNonOptionalField},
		},
		{
			...
			err: cdrjson.Error{Code: cdrjson.ErrNonOptionalDefault},
		},
		{
			...
			err: cdrjson.Error{Code: cdrjson.ErrMismatchedDefault},
		},
	}

	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		err := cdrjson.RegisterCmd(test.method, test.cmdFunc(),
			test.flags)
...
func TestMustRegisterCmdPanic(t *testing.T) {
	...
	cdrjson.MustRegisterCmd("panicme", 0, 0)
}
...
func TestRegisteredCmdMethods(t *testing.T) {
	...
	methods := cdrjson.RegisteredCmdMethods()
	if len(methods) == 0 {
