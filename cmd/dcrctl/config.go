...
const (
	...
	unusableFlags = cdrjson.UFWebsocketOnly | cdrjson.UFNotification
)

var (
	cdrdHomeDir            = cdrutil.AppDataDir("cdrd", false)
	cdrctlHomeDir          = cdrutil.AppDataDir("cdrctl", false)
	cdrwalletHomeDir       = cdrutil.AppDataDir("cdrwallet", false)
	defaultConfigFile      = filepath.Join(cdrctlHomeDir, "cdrctl.conf")
	defaultRPCServer       = "localhost"
	defaultWalletRPCServer = "localhost"
	defaultRPCCertFile     = filepath.Join(cdrdHomeDir, "rpc.cert")
	defaultWalletCertFile  = filepath.Join(cdrwalletHomeDir, "rpc.cert")
)
...
func listCommands() {
	...
	cmdMethods := cdrjson.RegisteredCmdMethods()
	categorized := make([][]string, numCategories)
	for _, method := range cmdMethods {
		flags, err := cdrjson.MethodUsageFlags(method)
		if err != nil {
		...
		usage, err := cdrjson.MethodUsageText(method)
		if err != nil {
			// This should never happen since the method was just
			// returned from the package, but be safe.
			continue
		...
		if flags&cdrjson.UFWalletOnly != 0 {
...
// config defines the configuration options for cdrctl.
//
...
// For this it tries to read the cdrd config file at its default path, and extract
// the RPC user and password from it.
func createDefaultConfigFile(destinationPath string) error {
	// Nothing to do when there is no existing cdrd conf file at the default
	// path to extract the details from.
	cdrdConfigPath := filepath.Join(cdrdHomeDir, "cdrd.conf")
	if !fileExists(cdrdConfigPath) {
		return nil
	}

	// Read cdrd.conf from its default path
	cdrdConfigFile, err := os.Open(cdrdConfigPath)
	if err != nil {
		return err
	}
	defer cdrdConfigFile.Close()
	content, err := ioutil.ReadAll(cdrdConfigFile)
	if err != nil {
		return err
	}
