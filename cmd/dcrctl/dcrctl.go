...
func main() {
	...
	method := args[0]
	usageFlags, err := cdrjson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := cdrjson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// cdrjson.Error as it reallistcally will always be since the
		// NewCmd function is only supposed to return errors of that
		// type.
		if jerr, ok := err.(cdrjson.Error); ok {
		...
		// The error is not a cdrjson.Error and this really should not
	...
	marshalledJSON, err := cdrjson.MarshalCmd("1.0", 1, cmd)
	if err != nil {
