...
cdrd provides a [JSON-RPC](http://json-rpc.org/wiki/specification) API that is
fully compatible with the original bitcoind/bitcoin-qt.  There are a few key
differences between cdrd and bitcoind as far as how RPCs are serviced:
* Unlike bitcoind that has the wallet and chain intermingled in the same process
  which leads to several issues, cdrd intentionally splits the wallet and chain
  ...
  talking directly to cdrd, only chain-related RPCs are available.  However both
  chain-related and wallet-related RPCs are available via
  [cdrwallet](https://github.com/commanderu/cdrwallet).
* cdrd is secure by default which means that the RPC connection is TLS-enabled
  by default
* cdrd provides access to the API through both
  [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29) requests and
  [Websockets](http://en.wikipedia.org/wiki/WebSocket)

Websockets are the preferred transport for cdrd RPC and are used by applications
such as [cdrwallet](https://github.com/commanderu/cdrwallet) for inter-process
communication with cdrd.  The websocket connection endpoint for cdrd is
`wss://your_ip_or_domain:37460/ws`.
...
during the development of cdrwallet.
...
The cdrd RPC server supports both [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29)
...
cdrd is `wss://your_ip_or_domain:37460/ws`.
...
to a cdrd RPC server:

* **rpcuser** is the full-access username configured for the cdrd RPC server
* **rpcpass** is the full-access password configured for the cdrd RPC server
* **rpclimituser** is the limited username configured for the cdrd RPC server
* **rpclimitpass** is the limited password configured for the cdrd RPC server
* **rpccert** is the PEM-encoded X.509 certificate (public key) that the cdrd
  server is configured with.  It is automatically generated by cdrd and placed
  in the cdrd home directory (which is typically `%LOCALAPPDATA%\cdrd` on
  Windows and `~/.cdrd` on POSIX-like OSes)

**NOTE:** As mentioned above, cdrd is secure by default which means the RPC
...
The cdrd RPC server uses HTTP [basic access authentication](http://en.wikipedia.org/wiki/Basic_access_authentication) with the **rpcuser**
...
cdrd comes with a separate utility named `cdrctl` which can be used to issue
these RPC commands via HTTP POST requests to cdrd after configuring it with the
...
API compatible with the original cdrd client.
...
|24|[getwork](#getwork)|N|Returns formatted hash data to work on or checks and submits solved data.<br /><br />NOTE: Since cdrd does not have the wallet integrated to provide payment addresses, cdrd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
...
|27|[sendrawtransaction](#sendrawtransaction)|Y|Submits the serialized, hex-encoded transaction to the local peer and relays it to the network.<br /><br />NOTE: cdrd does not yet implement the `allowhighfees` parameter, so it has no effect|
|28|[setgenerate](#setgenerate) |N|Set the server to generate coins (mine) or not.<br/>NOTE: Since cdrd does not have the wallet integrated to provide payment addresses, cdrd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|29|[stop](#stop)|N|Shutdown cdrd.|
|30|[submitblock](#submitblock)|Y|Attempts to submit a new serialized, hex-encoded block to the network.|
|31|[validateaddress](#validateaddress)|Y|Verifies the given address is valid.  NOTE: Since cdrd does not have a wallet integrated, cdrd will only return whether the address is valid or not.|
...
|35|[getcurrentnet](#getcurrentnet)|Y|Get commanderu network cdrd is running on.|
...
|Parameters|1. `transaction inputs`:  `(JSON array, required)` json array of json objects.<br />`hash`: `(string, required)` the hash of the input.</br> `vout`: (numeric, required) the specific output of the input transaction to redeem transaction.<br /><br />`[{"txid": "hash", "vout": n}, ...]`<br /><br />2. `addresses and amounts`: `(JSON object, required)` - json object with addresses as keys and amounts as values.</br>`address`:   `(numeric, required)` the address to send to as the key and the amount in cdr as the value.<br /><br />`{"address": n.nnn, ...}`|
...
|Returns|`(json object)`<br />`asm`: `(string)` disassembly of the script (absent for nonstandard scripts).<br />`reqSigs`: `(numeric)` the number of required signatures.<br />`type`: `(string)` the type of the script (e.g. 'pubkeyhash').<br />`addresses`: `(json array of string)` the commanderu addresses associated with this script.<br />`p2sh`: `(string)` the script hash for use in pay-to-script-hash transactions.<br /><br />`{ "asm": "asm", "reqSigs": n, "type": "scripttype", "addresses": [...], "p2sh": "scripthash"}`|
...
|Example Return|`[{"addr": "178.172.xxx.xxx:9108", "services": "00000001", "lastrecv": 1388183523, "lastsend": 1388185470, "bytessent": 287592965, "bytesrecv": 780340, "conntime": 1388182973, "pingtime": 405551, "pingwait": 183023, "version": 70001, "subver": "/cdrd:0.4.0/", "inbound": false, "startingheight": 276921, "currentheight": 276955, "syncnode": true }, ...]`|
[Return to Overview](#MethodOverview)<br />
...
|Description|Returns information about a transaction given its hash.|
|Notes|Since cdrd does not have the wallet integrated to provide payment addresses, cdrd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.
...
|Notes|NOTE: Since cdrd does not have the wallet integrated to provide payment addresses, cdrd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|Returns|Nothing|
[Return to Overview](#MethodOverview)<br />
...
|Notes|cdrd does not yet implement the `allowhighfees` parameter, so it has no effect.|
...
|Method|stop|
|Parameters|None|
|Description|Shutdown cdrd.|
|Returns|`"cdrd stopping."` (string)|
[Return to Overview](#MethodOverview)<br />
...
|Parameters|1. `address`: `(string, required)` commanderu address.|
|Description|Verify an address is valid.|
|Returns|`(json object)`<br />`isvalid`: `(bool)` whether or not the address is valid.<br />`address`: `(string)` the commanderu address validated.<br /><br />`{"isvalid": true or false,"address": "commanderuaddress"}`|
[Return to Overview](#MethodOverview)<br />
...
|Description|Verifies the block chain database. The actual checks performed by the `checklevel` parameter is implementation specific. <br /><br />For cdrd this is: <br />`checklevel=0` - Look up each block and ensure it can be loaded from the database.<br />`checklevel=1` - Perform basic context-free sanity checks on each block.|
|Notes|cdrd currently only supports `checklevel` 0 and 1, but the default is still 3 for compatibility.  Per the information in the Parameters section above, higher levels are automatically clamped to the highest supported level, so this means the default is effectively 1 for cdrd.|
...
|Description|Dynamically changes the debug logging level.<br />The levelspec can either a debug level or of the form `<subsystem>=<level>,<subsystem2>=<level2>,...`<br />The valid debug levels are `trace`, `debug`, `info`, `warn`, `error`, and `critical`.<br />The valid subsystems are `AMGR`, `ADXR`, `BCDB`, `BMGR`, `cdrD`, `CHAN`, `DISC`, `PEER`, `RPCS`, `SCRP`, `SRVR`, and `TXMP`.<br />Additionally, the special keyword `show` can be used to get a list of the available subsystems.|
|Returns|`string`|
|Example Return|`Done.`|
|Example `show` Return|`Supported subsystems [AMGR ADXR BCDB BMGR cdrD CHAN DISC PEER RPCS SCRP SRVR TXMP]`|
...
|Parameters|None|
|Description|Get commanderu network cdrd is running on.|
|Returns|`numeric`|
...
|Parameters|1. `Addresses`: `(JSON array, required)`<br />`commanderuaddress`: `(string)` the commanderu address.<br /><br />`["commanderuaddress", ...]`|
...
|Description|Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this cdrd instance) and when such a transaction first appears in a newly-attached block.|
|Returns|Nothing|
[Return to Overview](#WSMethodOverview)<br />
...
[rpcclient](https://github.com/commanderu/cdrd/tree/master/rpcclient) package.
...
[rpcclient](https://github.com/commanderu/cdrd/tree/master/rpcclient) package to connect with
a cdrd instance via Websockets, issues [getblockcount](#getblockcount) to
...
func main() {
	...
	// generated by cdrd when it starts the RPC server and doesn't already
	// have one.
...
The following is an example Go application which uses the
[rpcclient](https://github.com/commanderu/cdrd/tree/master/rpcclient) package to connect with
a cdrd instance via Websockets, issues [getblock](#getblock) to retrieve
...
The following is an example Go application which uses the
[rpcclient](https://github.com/commanderu/cdrd/tree/master/rpcclient) package to connect with
a cdrd instance via Websockets and registers for
...
(can be installed with `npm install ws`) to connect with a cdrd instance,
...
// generated by cdrd when it starts the RPC server and doesn't already
// have one.
var cert = fs.readFileSync('/path/to/cdrd/appdata/rpc.cert');
var user = "yourusername";
var password = "yourpassword";


// Initiate the websocket connection.  The cdrd generated certificate acts as
// its own certificate authority, so it needs to be specified in the 'ca' array
// for the certificate to properly validate.
var ws = new WebSocket('wss://127.0.0.1:37460/ws', {
  headers: {
