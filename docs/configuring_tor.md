...
cdrd provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, cdrd supports a hybrid,
...
While it is easier to only run as a client, it is more beneficial to the commanderu
...
Configuring cdrd as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the cdrd command
line or in the cdrd configuration file.  Typically the Tor proxy address will be
...
By default, cdrd assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since cdrd will act as the server.
...
```bash
$ ./cdrd --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
cdrd provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the cdrd command line or in the cdrd
...
```bash
$ ./cdrd --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
cdrd provides support for Tor stream isolation by using the `--torisolation`
...
```bash
$ ./cdrd --proxy=127.0.0.1:9050 --torisolation
```
